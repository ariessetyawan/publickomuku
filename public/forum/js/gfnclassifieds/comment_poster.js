 ...
 */
!function(c,b,a,d){if(typeof GFNClassifieds=="undefined"){GFNClassifieds={}}GFNClassifieds.CommentPoster=function(e){e.bind({AutoValidationComplete:function(g){if(g.ajaxData.comments){var h=g.ajaxData.comments;new XenForo.ExtLoader(g.ajaxData,function(){c.each(h,function(j,i){c(i).xfInsert("prependTo","#NewComments")})});e.find("input[name=last_date]").val(g.ajaxData.lastCommentDate);e.find("input:submit").removeAttr("disabled").removeClass("disabled");var f=e.find("textarea").val("").data("XenForo.BbCodeWysiwygEditor");if(f){f.resetEditor()}}}})};XenForo.register("#QuickReply","GFNClassifieds.CommentPoster")}(jQuery,this,document);