...
class XenForo_Search_SourceHandler_MySqlFt extends XenForo_Search_SourceHandler_Abstract
{
	...
	protected function _pushToIndex($record)
	{
		...
		$this->_getDb()->query('
			REPLACE ' . ($this->_isRebuild || !XenForo_Application::get('options')->enableInsertDelayed ? '' : 'DELAYED') . ' INTO kmk_search_index
	...
	public function deleteIndex($contentType = null)
	{
		if (!$contentType)
		{
			XenForo_Application::getDb()->query('TRUNCATE TABLE kmk_search_index');
		}
	}
	...
	)
	{
		...
		return $db->fetchAll("
			SELECT $selectFields
			FROM kmk_search_index AS search_index
			" . implode("\n", $joins) . "
