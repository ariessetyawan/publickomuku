...
class XenForo_Model_Admin extends XenForo_Model
{
	...
	public function getAdminById($userId)
	{
		return $this->_getDb()->fetchRow('
			SELECT admin.*, user.*
			FROM kmk_admin AS admin
			INNER JOIN kmk_user AS user ON (user.user_id = admin.user_id)
			WHERE admin.user_id = ?
		', $userId);
	}
	...
	{
		return $this->_getDb()->fetchOne('SELECT COUNT(*) FROM kmk_admin');
	}
	...
	public function getAllAdminPermissions()
	{
		return $this->fetchAllKeyed('
			...
			FROM kmk_admin_permission AS admin_permission
	...
	public function getAdminPermissionsForUser($userId)
	{
		return $this->fetchAllKeyed('
			...
			FROM kmk_admin_permission AS admin_permission
			LEFT JOIN kmk_admin_permission_entry AS admin_permission_entry ON
	...
	public function updateUserAdminPermissions($userId, array $newPermissions)
	{
		...
		$db->delete('kmk_admin_permission_entry', 'user_id = ' . $db->quote($userId));

		$keyCache = array();
		foreach (array_unique($newPermissions) AS $newPermissionId)
		{
			$db->insert('kmk_admin_permission_entry', array(
				'user_id' => $userId,
	...
	public function rebuildUserAdminPermissionCache()
	{
		...
		$permissionsSql = $db->query('
			...
			FROM kmk_admin_permission_entry AS admin_permission_entry
			INNER JOIN kmk_admin_permission AS admin_permission ON
				(admin_permission.admin_permission_id = admin_permission_entry.admin_permission_id)
		');
	...
	{
		$db = $this->_getDb();
		$db->delete('kmk_admin_permission', 'addon_id = ' . $db->quote($addOnId));
	}
