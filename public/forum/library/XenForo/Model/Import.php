...
class XenForo_Model_Import extends XenForo_Model
{
	...
	{
		$this->_getDb()->query('
			TRUNCATE TABLE kmk_import_log
		');
		$this->_getDb()->query('ALTER TABLE kmk_import_log ENGINE = InnoDB');
	}
	...
	public function archiveImportLog($archiveTableName, &$error)
	{
		...
		try
		{
			$db->query("ALTER TABLE kmk_import_log RENAME {$archiveTableName}");

			$tables = XenForo_Install_Data_MySql::getTables();

			$db->query($tables['kmk_import_log']);
		}
	...
	{
		$logTable = (defined('IMPORT_LOG_TABLE') ? IMPORT_LOG_TABLE : 'kmk_import_log');
	...
	public function importUser($oldId, array $info, &$failedKey = '', $handleAdmin = true)
	{
		...
		if (isset($info['authentication']))
		{
			...
			$dw->set('data', serialize($info['authentication']['data']), 'kmk_user_authenticate');
		}
	...
	public function importFollowing($userId, array $followUserIds)
	{
		...
		{
			$db->query('
				INSERT IGNORE INTO kmk_user_follow
	...
	public function importIgnored($userId, array $ignoreUserIds)
	{
		...
		{
			$db->query('
				INSERT IGNORE INTO kmk_user_ignored
	...
	{
		$db = $this->_getDb();
		$db->insert('kmk_user_upgrade_active', $info);
	...
	public function importConversation($oldId, array $conversation, array $recipients, array $messages)
	{
		...
		if ($conversationId)
		{
			...
			$db->update('kmk_conversation_master', $conversationUpdate, 'conversation_id = ' . $db->quote($conversationId));

			foreach ($recipients AS $userId => $info)
			{
				$db->insert('kmk_conversation_recipient', array(
				...
				if ($info['recipient_state'] == 'active')
				{
					...
					$db->insert('kmk_conversation_user', $recipientUser);
				}
			}
		}
	...
	public function importProfilePost($oldId, array $info)
	{
		...
		if ($profilePostId)
		{
			...
			{
				$this->_getDb()->query('
					INSERT IGNORE INTO kmk_moderation_queue
			...
			if ($ip)
			{
				...
				if ($ipId)
				{
					$this->_getDb()->update('kmk_profile_post',
						array('ip_id' => $ipId),
	...
	public function insertNodePermissionEntries($nodeId, $userGroupId, $userId, array $perms)
	{
		...
		foreach ($perms AS $groupId => $groupPerms)
		{
			foreach ($groupPerms AS $permId => $value)
			{
				...
				$db->query('
					INSERT INTO kmk_permission_entry_content
	...
	public function importGlobalModerator($oldId, array $info)
	{
		...
		if ($userId)
		{
			...
			$db = $this->_getDb();
			$db->update('kmk_user', array('is_moderator' => 1), 'user_id = ' . $db->quote($userId));
		}
	...
	public function importNodeModerator($oldNodeId, $oldUserId, array $info)
	{
		...
		if ($dw->save())
		{
			...
			$db = $this->_getDb();
			$db->update('kmk_user', array('is_moderator' => 1), 'user_id = ' . $db->quote($dw->get('user_id')));
		}
		else
		{
			$newId = false;
	...
	public function importTag($tag, $contentType, $contentId, array $content)
	{
		...
		$result = $db->query('
			INSERT IGNORE INTO kmk_tag_content
	...
	public function importThreadWatch($userId, $threadId, $emailSubscribe, $deferred = false)
	{
		...
		$this->_getDb()->query('
			INSERT IGNORE INTO kmk_thread_watch
	...
	{
		$this->_getDb()->query('
			INSERT IGNORE INTO kmk_poll_vote
	...
	protected $_supportedLikeTypes = array(
		'post' => array('kmk_post', 'likes', 'like_users', 'post_id'),
		'profile_post' => array('kmk_profile_post', 'likes', 'like_users', 'profile_post_id'),
		'profile_post_comment' => array('kmk_profile_post_comment', 'likes', 'like_users', 'profile_post_comment_id')
	);
	...
	protected $_supportedWarningTypes = array(
		'post' => array('kmk_post', 'warning_id', 'warning_message', 'post_id'),
		'profile_post' => array('kmk_profile_post', 'warning_id', 'warning_message', 'profile_post_id'),
	);
	...
	public function importLike($contentType, $contentId, $contentUserId, $likeUserId, $likeDate, $updateSerialized = false)
	{
		...
		$result = $db->query('
			INSERT IGNORE INTO kmk_liked_content
