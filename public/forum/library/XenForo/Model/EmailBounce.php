...
class XenForo_Model_EmailBounce extends XenForo_Model
{
	public function processBounceEmail($rawMessage)
	{
		...
		if ($recipient)
		{
			$userId = $this->_getDb()->fetchOne('SELECT user_id FROM kmk_user WHERE email = ?', $recipient);
			if (!$userId)
			{
				$userId = null;
			}
	...
	public function takeBounceAction($userId, $bounceType, $bounceDate)
	{
		...
		{
			$this->_getDb()->query("
				INSERT INTO kmk_email_bounce_soft
	...
	{
		$this->_getDb()->insert('kmk_email_bounce_log', array(
	...
	public function getEmailBounceLogs(array $conditions = array(), array $fetchOptions = array())
	{
		...
		return $this->fetchAllKeyed($this->limitQueryResults(
			"
				...
				FROM kmk_email_bounce_log AS email_bounce
				LEFT JOIN kmk_user AS user ON (user.user_id = email_bounce.user_id)
	...
	public function pruneEmailBounceLogs($cutOff = null)
	{
		...
		$db = $this->_getDb();
		return $db->delete('kmk_email_bounce_log', 'log_date < ' . $db->quote($cutOff));
	}
	...
	public function pruneSoftBounceHistory($cutOff = null)
	{
		...
		$db = $this->_getDb();
		return $db->delete('kmk_email_bounce_soft', 'bounce_date < ' . $db->quote($sqlDate));
	}
