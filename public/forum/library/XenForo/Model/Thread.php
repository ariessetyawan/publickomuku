...
class XenForo_Model_Thread extends XenForo_Model
{
	...
	public function getThreadsByIds(array $threadIds, array $fetchOptions = array())
	{
		...
		return $this->fetchAllKeyed('
			SELECT thread.*
				' . $joinOptions['selectFields'] . '
			FROM kmk_thread AS thread' . $joinOptions['joinTables'] . '
	...
	public function prepareThreadFetchOptions(array $fetchOptions)
	{
		...
		if (!empty($fetchOptions['join']))
		{
			...
			if ($fetchOptions['join'] & self::FETCH_LAST_POST_AVATAR)
			{
				...
				$joinTables .= '
					LEFT JOIN kmk_user AS last_post_user ON
		...
		if (isset($fetchOptions['replyBanUserId']))
		{
			if (!empty($fetchOptions['replyBanUserId']))
			{
				...
				$joinTables .= '
					LEFT JOIN kmk_thread_reply_ban AS reply_ban
		...
		if (isset($fetchOptions['forumWatchUserId']))
		{
			if (!empty($fetchOptions['forumWatchUserId']))
			{
				...
				$joinTables .= '
					LEFT JOIN kmk_forum_watch AS forum_watch
	...
	public function getThreads(array $conditions, array $fetchOptions = array())
	{
		...
		return $this->fetchAllKeyed($this->limitQueryResults(
			'
				...
				FROM kmk_thread AS thread ' . $forceIndex . '
	...
	public function getUnreadThreadIds($userId, array $fetchOptions = array(), $watchedOnly = false)
	{
		...
		{
			$joinExtra = '
				LEFT JOIN kmk_forum_watch AS forum_watch ON (forum_watch.node_id = thread.node_id AND forum_watch.user_id = ' . $userId . ')
				LEFT JOIN kmk_thread_watch AS thread_watch ON (thread_watch.thread_id = thread.thread_id AND thread_watch.user_id = ' . $userId . ')
			';
	...
	public function replaceThreadUserPostCounters($threadId, array $counters, $userId = null)
	{
		...
		$db->delete('kmk_thread_user_post', 'thread_id = ' . $db->quote($threadId) . $userIdConstraint);
		...
		foreach ($counters AS $userId => $count)
		{
			...
			$db->insert('kmk_thread_user_post', array(
				'thread_id' => $threadId,
				'user_id' => $userId,
				'post_count' => $count
			));
	...
	public function modifyThreadUserPostCount($threadId, $userId, $modifyValue)
	{
		...
		if ($postCount === false)
		{
			...
			$db->query("
				INSERT INTO kmk_thread_user_post
		...
		else
		{
			...
			if ($finalValue <= 0)
			{
				$db->delete('kmk_thread_user_post', $condition);
				return false;
			}
			else
			{
				$db->update('kmk_thread_user_post', array('post_count' => $finalValue), $condition);
				return $finalValue;
			}
		}
	}
	...
	public function mergeThreads(array $threads, $targetThreadId, array $options = array())
	{
		...
		$db->query("
			UPDATE IGNORE kmk_thread_watch SET
				thread_id = ?
			WHERE thread_id IN ($idsQuoted)
		", array($targetThreadId));
		$db->query("
			UPDATE IGNORE kmk_tag_content SET
				content_id = ?
			WHERE content_type = 'thread' AND content_id IN ($idsQuoted)
		", array($targetThreadId));
		$db->query("
			UPDATE IGNORE kmk_thread_reply_ban SET
				thread_id = ?
			WHERE thread_id IN ($idsQuoted)
		", array($targetThreadId));
		...
		if ($options['redirect'])
		{
			...
			$db->delete('kmk_thread_watch', "thread_id IN ($idsQuoted)");
			$db->delete('kmk_thread_reply_ban', "thread_id IN ($idsQuoted)");
			$db->delete('kmk_thread_user_post', "thread_id IN ($idsQuoted)");
			$db->delete('kmk_poll', "content_type = 'thread' AND content_id IN ($idsQuoted)");
	...
	{
		$this->_getDb()->query('
			INSERT ' . (XenForo_Application::get('options')->enableInsertDelayed ? 'DELAYED' : '') . ' INTO kmk_thread_view
				(thread_id)
			VALUES
				(?)
		', $threadId);
	...
	public function updateThreadViews()
	{
		...
		$db->query('
			...
			INNER JOIN (
				...
				FROM kmk_thread_view
				GROUP BY thread_id
			) AS kmk_tv ON (kmk_tv.thread_id = kmk_thread.thread_id)
			SET kmk_thread.view_count = kmk_thread.view_count + kmk_tv.total
		');

		$db->query('TRUNCATE TABLE kmk_thread_view');
	}
	...
	public function getThreadReplyBans(array $conditions, array $fetchOptions = array())
	{
		...
		return $this->fetchAllKeyed($this->limitQueryResults('
			SELECT reply_ban.*,
				...
				' . $sqlClauses['selectFields'] . '
			FROM kmk_thread_reply_ban AS reply_ban
			LEFT JOIN kmk_user AS user ON (user.user_id = reply_ban.user_id)
			LEFT JOIN kmk_user AS ban_user ON (ban_user.user_id = reply_ban.ban_user_id)
			' . $sqlClauses['joinTables'] . '
			WHERE ' . $whereConditions . '
			' . $sqlClauses['orderClause'] . '
			', $limitOptions['limit'], $limitOptions['offset']
	...
	public function insertThreadReplyBan(array $thread, array $user, $expiryDate = null, $reason = '', $sendAlert = false, $banUserId = null)
	{
		...
		$this->_getDb()->query("
			INSERT INTO kmk_thread_reply_ban
	...
	public function deleteThreadReplyBan(array $thread, array $user)
	{
		$res = $this->_getDb()->query("
			DELETE FROM kmk_thread_reply_ban
			WHERE thread_id = ?
				AND user_id = ?
	...
	{
		return (int)$this->_getDb()->fetchOne('SELECT MIN(post_date) FROM kmk_thread');
	}
