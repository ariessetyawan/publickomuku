...
class XenForo_Model_Permission extends XenForo_Model
{
	...
	public function getAllPermissionsWithValues($userGroupId = 0, $userId = 0)
	{
		...
		return $this->_getDb()->fetchAll('
			...
			FROM kmk_permission AS permission
			LEFT JOIN kmk_permission_entry AS entry ON
	...
	public function getContentPermissionsWithValues($contentTypeId, $contentId, $permissionGroups, $userGroupId = 0, $userId = 0)
	{
		...
		return $db->fetchAll('
			...
			FROM kmk_permission AS permission
			LEFT JOIN kmk_permission_entry_content AS entry_content ON
	...
	public function getAllPermissionInterfaceGroups()
	{
		return $this->fetchAllKeyed('
			SELECT *
			FROM kmk_permission_interface_group
			ORDER BY display_order
		', 'interface_group_id');
	}
	...
	public function getContentPermissionEntryById($id)
	{
		return $this->_getDb()->fetchRow('
			SELECT *
			FROM kmk_permission_entry_content
			WHERE permission_entry_id = ?
		', $id);
	}
	...
	public function getAllPermissionEntriesGrouped()
	{
		...
		$entryResult = $this->_getDb()->query('
			...
			FROM kmk_permission_entry AS entry
			INNER JOIN kmk_permission AS permission ON
	...
	public function getAllContentPermissionEntriesByTypeGrouped($permissionType)
	{
		...
		$entryResult = $this->_getDb()->query('
			...
			FROM kmk_permission_entry_content AS entry_content
			INNER JOIN kmk_permission AS permission ON
	...
	public function getPermissionCombinationsByUserGroupId($userGroupId)
	{
		return $this->fetchAllKeyed('
			...
			FROM kmk_permission_combination_user_group AS combination_user_group
			INNER JOIN kmk_permission_combination AS combination ON
	...
	public function deletePermissionCombination($combinationId)
	{
		...
		$db->delete('kmk_permission_combination', $combinationCondition);
		$db->delete('kmk_permission_combination_user_group', $combinationCondition);
		$db->delete('kmk_permission_cache_content', $combinationCondition);
	}
	...
	public function deleteUnusedPermissionCombinations()
	{
		...
		$combinationIds = $db->fetchCol("
			...
			FROM kmk_permission_combination AS p
			LEFT JOIN (SELECT DISTINCT u.permission_combination_id FROM kmk_user AS u) AS up
	...
	public function findOrCreatePermissionCombination($userId, array $userGroupIds, $buildOnCreate = true)
	{
		...
		$db->insert('kmk_permission_combination', $combination);
		$combination['permission_combination_id'] = $db->lastInsertId('kmk_permission_combination', 'permission_combination_id');

		foreach (explode(',', $userGroupList) AS $userGroupId)
		{
			$db->insert('kmk_permission_combination_user_group', array(
				'user_group_id' => $userGroupId,
	...
	*/
	...
	public function rebuildPermissionCombination(array $combination, array $permissionsGrouped, array $entries)
	{
		...
		$db->update('kmk_permission_combination', array(
	...
	public function rebuildContentPermissionCombination(array $combination, array $permissionsGrouped, array &$permCache)
	{
		...
		foreach ($contentHandlers AS $contentTypeId => $handler)
		{
			...
			foreach ($cacheEntries AS $contentId => $entry)
			{
				...
				{
					$db->query('
						INSERT INTO kmk_permission_cache_content
	...
	public function deletePermissionsForAddOn($addOnId)
	{
		...
		$db->delete('kmk_permission', $addOnClause);
		$db->delete('kmk_permission_group', $addOnClause);
		$db->delete('kmk_permission_interface_group', $addOnClause);
	}
	...
	public function getUsersWithGlobalUserPermissions()
	{
		return $this->fetchAllKeyed('
			SELECT user.*
			FROM kmk_permission_entry AS permission_entry
	...
	public function getUsersWithContentUserPermissions($contentType, $contentId)
	{
		return $this->fetchAllKeyed('
			SELECT user.*
			FROM kmk_permission_entry_content AS permission_entry_content
			INNER JOIN kmk_user AS user ON
	...
	public function getUserCombinationsWithContentPermissions($contentType, $contentId = null)
	{
		...
		return $db->fetchAll('
			...
			FROM kmk_permission_entry_content AS entry
			INNER JOIN kmk_permission AS permission ON
				(permission.permission_group_id = entry.permission_group_id
				AND permission.permission_id = entry.permission_id)
			LEFT JOIN kmk_user AS user ON (user.user_id = entry.user_id AND entry.user_id > 0)
			LEFT JOIN kmk_user_group AS user_group ON (user_group.user_group_id = entry.user_group_id AND entry.user_group_id > 0)
