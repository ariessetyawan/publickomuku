...
class XenForo_Model_Smilie extends XenForo_Model
{
	...
	public function prepareSmilie(array $smilie, $getSmilieText = false)
	{
		...
		{
			$smilie['sprite_params'] = XenForo_Helper_Php::safeUnserialize($smilie['sprite_params']);
		}
	...
	public function getSmiliesXml(array $smilieIds)
	{
		if ($smilieIds)
		{
			$smilies = $this->fetchAllKeyed('
				SELECT kmk_smilie.*,
					kmk_smilie_category.display_order AS smilie_category_order
				FROM kmk_smilie
				LEFT JOIN kmk_smilie_category ON
					(kmk_smilie_category.smilie_category_id = kmk_smilie.smilie_category_id)
				WHERE kmk_smilie.smilie_id IN (' . $this->_getDb()->quote($smilieIds) . ')
				ORDER BY kmk_smilie_category.display_order, kmk_smilie.display_order, kmk_smilie.title
			', 'smilie_id');
		}
		else
		{
	...
	protected function _getExistingSmilieCategoryIdsByTitle()
	{
		return $this->_getDb()->fetchPairs("
			...
			FROM kmk_smilie_category AS sc
			INNER JOIN kmk_phrase AS p ON
