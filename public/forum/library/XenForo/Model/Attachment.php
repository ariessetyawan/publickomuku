...
class XenForo_Model_Attachment extends XenForo_Model
{
	...
	public function getAttachmentsByContentIds($contentType, array $contentIds)
	{
		return $this->fetchAllKeyed('
			...
			INNER JOIN kmk_attachment_data AS data ON
	...
	public function getAttachments(array $conditions = array(), array $fetchOptions = array())
	{
		...
		return $this->fetchAllKeyed($this->limitQueryResults(
			'
				...
				FROM kmk_attachment AS attachment
				INNER JOIN kmk_attachment_data AS attachment_data ON
	...
	{
		$this->_getDb()->query('
			INSERT ' . (XenForo_Application::get('options')->enableInsertDelayed ? 'DELAYED' : '') . ' INTO kmk_attachment_view
				(attachment_id)
			VALUES
				(?)
		', $attachmentId);
	...
	public function updateAttachmentViews()
	{
		...
		$db->query('
			UPDATE kmk_attachment
			INNER JOIN (
				SELECT attachment_id, COUNT(*) AS total
				FROM kmk_attachment_view
				GROUP BY attachment_id
			) AS kmk_av ON (kmk_av.attachment_id = kmk_attachment.attachment_id)
			SET kmk_attachment.view_count = kmk_attachment.view_count + kmk_av.total
		');

		$db->query('TRUNCATE TABLE kmk_attachment_view');
	}
