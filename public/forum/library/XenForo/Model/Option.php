...
class XenForo_Model_Option extends XenForo_Model
{
	...
	public function getOptionGroupList(array $fetchOptions = array())
	{
		...
		return $this->fetchAllKeyed('
			...
			FROM kmk_option_group AS option_group
	...
	public function getOptionsInGroup($groupId, array $fetchOptions = array())
	{
		...
		return $this->fetchAllKeyed('
			SELECT kmk_option.*,
				...
				' . $joinOptions['selectFields'] . '
			FROM kmk_option_group_relation AS relation
			INNER JOIN kmk_option ON
				(relation.option_id = kmk_option.option_id)
				' . $joinOptions['joinTables'] . '
	...
	public function prepareOptionConditions(array $conditions, array $fetchOptions = array())
	{
		...
		{
			$sqlConditions[] = 'kmk_option.option_id IN (' . $db->quote($conditions['optionIds']) . ')';
		}

		if (!empty($conditions['addOnId']))
		{
			$sqlConditions[] = 'kmk_option.addon_id = ' . $db->quote($conditions['addOnId']);
		}

		if (!empty($conditions['adminQuickSearch']))
		{
			$searchStringSql = 'CONVERT(kmk_option.option_id USING utf8) LIKE ' . XenForo_Db::quoteLike($conditions['adminQuickSearch']['searchText'], 'lr');

			if (!empty($conditions['adminQuickSearch']['phraseMatches']))
			{
				$sqlConditions[] = '(' . $searchStringSql . ' OR CONVERT(kmk_option.option_id USING utf8) IN (' . $db->quote($conditions['adminQuickSearch']['phraseMatches']) . '))';
			}
			else
			{
	...
	{
		$choices = array(
			'option_id' => 'kmk_option.option_id',
		);
	...
	public function getOptions(array $conditions, array $fetchOptions = array())
	{
		...
		$orderClause = $this->prepareOptionOrderOptions($fetchOptions, 'kmk_option.option_id');
		$joinOptions = $this->prepareOptionFetchOptions($fetchOptions);
	...
	public function prepareOptionFetchOptions(array $fetchOptions)
	{
		...
		if (!empty($fetchOptions['join']))
		{
			...
			{
				$selectFields .= ',
					addon.*, addon.title AS addon_title, kmk_option.addon_id';
				$joinTables .= '
					LEFT JOIN kmk_addon AS addon ON
						(addon.addon_id = kmk_option.addon_id)';
			}
	...
	public function getGroupForOption($optionId)
	{
		return $this->_getDb()->fetchRow('
			SELECT *
			FROM kmk_option_group_relation AS rel
			INNER JOIN kmk_option_group AS grp ON
	...
	public function getOptionRelationsByOptionId($optionId)
	{
		return $this->_getDb()->fetchPairs('
			...
			FROM kmk_option_group_relation
			WHERE option_id = ?
		', $optionId);
	}
	...
	public function deleteOptionsInGroup($groupId)
	{
		...
		if ($options)
		{
			...
			{
				$db->delete('kmk_option', 'option_id IN (' . $db->quote($singleGroupOptions) . ')');
			...
			$db->delete('kmk_option_group_relation',
	...
	public function deleteOptionsForAddOn($addOnId)
	{
		...
		$db->query('
			DELETE FROM kmk_option_group_relation
			WHERE option_id IN (
				SELECT option_id
				FROM kmk_option
				WHERE addon_id = ?
		...
		', $addOnId);
		$db->delete('kmk_option', 'addon_id = ' . $db->quote($addOnId));
		$db->delete('kmk_option_group', 'addon_id = ' . $db->quote($addOnId));
	}
