...
class XenForo_Model_User extends XenForo_Model
{
	...
	public static $defaultGuestGroupId = 9;
	public static $defaultRegisteredGroupId = 7;
	...
	/**
	 * Stores the unserialized value of kmk_user.ignored for each user that has been inspected
	 *
	...
	public function prepareUserFetchOptions(array $fetchOptions)
	{
		...
		if (!empty($fetchOptions['join']))
		{
			if ($fetchOptions['join'] & self::FETCH_USER_PROFILE)
			{
				$selectFields .= ',
					user_profile.*';
				$joinTables .= '
					LEFT JOIN kmk_user_profile AS user_profile ON
			...
			if ($fetchOptions['join'] & self::FETCH_USER_PRIVACY)
			{
				$selectFields .= ',
					user_privacy.*';
				$joinTables .= '
					LEFT JOIN kmk_user_privacy AS user_privacy ON
		...
		if (!empty($fetchOptions['customFields']) && is_array($fetchOptions['customFields']))
		{
			foreach ($fetchOptions['customFields'] AS $customFieldId => $value)
			{
				...
				{
					$joinTables .= "
						LEFT JOIN kmk_user_field_value AS user_field_value_$customFieldId ON
				...
				else
				{
					...
					$joinTables .= "
						INNER JOIN kmk_user_field_value AS user_field_value_$customFieldId ON
							(user_field_value_$customFieldId.user_id = user.user_id
							AND user_field_value_$customFieldId.field_id = " . $this->_getDb()->quote($customFieldId) . "
	...
	public function updateSessionActivity($userId, $ip, $controllerName, $action, $viewState, array $inputParams, $viewDate = null, $robotKey = '')
	{
		...
		{
			$this->_getDb()->query('
				INSERT INTO kmk_session_activity
	...
	public function deleteSessionActivity($userId, $ip)
	{
		...
		$db = $this->_getDb();
		$db->delete('kmk_session_activity', 'user_id = ' . $db->quote($userId) . ' AND unique_key = ' . $db->quote($uniqueKey));
	}
	...
	public function addUserGroupChange($userId, $key, $addGroups)
	{
		...
		{
			$db->query('
				INSERT INTO kmk_user_group_change
	...
	public function removeUserGroupChange($userId, $key)
	{
		...
		if ($success)
		{
			$db->delete('kmk_user_group_change',
	...
	{
		$db = $this->_getDb();
		$db->delete('kmk_user_group_change', 'change_key = ' . $db->quote($key));
	}
	...
	public function getFollowedUserProfiles($userId, $maxResults = 0, $orderBy = 'user.username')
	{
		$sql = '
			...
			INNER JOIN kmk_user_profile AS user_profile ON
				(user_profile.user_id = user.user_id)
			INNER JOIN kmk_user_option AS user_option ON
				(user_option.user_id = user.user_id)
	...
	{
		return (int)$this->_getDb()->fetchOne('SELECT MIN(register_date) FROM kmk_user');
	}
	...
	public static $userContentChanges = array(
		'kmk_admin_log' => array(array('user_id', false, false)),
		'kmk_attachment_data' => array(array('user_id')),
		'kmk_conversation_master' => array(array('user_id', 'username'), array('last_message_user_id', 'last_message_username')),
		'kmk_conversation_message' => array(array('user_id', 'username')),
		'kmk_conversation_recipient' => array(array('user_id', false, false)),
		'kmk_conversation_user' => array(array('owner_user_id', false, false), array('last_message_user_id', 'last_message_username')),
		'kmk_deletion_log' => array(array('delete_user_id', 'delete_username')),
		'kmk_edit_history' => array(array('edit_user_id')),
		'kmk_forum_watch' => array(array('user_id', false, false)),
		'kmk_ip' => array(array('user_id', false, false)),
		'kmk_liked_content' => array(array('like_user_id'), array('content_user_id')),
		'kmk_news_feed' => array(array('user_id', 'username', false)),
		'kmk_poll_vote' => array(array('user_id', false, false)),
		'kmk_post' => array(array('user_id', 'username')),
		'kmk_profile_post' => array(array('profile_user_id', false, false), array('user_id', 'username')),
		'kmk_profile_post_comment' => array(array('user_id', 'username')),
		'kmk_report' => array(array('content_user_id'), array('last_modified_user_id', 'last_modified_username')),
		'kmk_report_comment' => array(array('user_id', 'username')),
		'kmk_spam_cleaner_log' => array(array('user_id', 'username'), array('applying_user_id', 'applying_username')),
		'kmk_thread' => array(array('user_id', 'username'), array('last_post_user_id', 'last_post_username')),
		'kmk_forum' => array(array('last_post_user_id', 'last_post_username')),
		'kmk_thread_reply_ban' => array(array('user_id', false, false), array('ban_user_id')),
		'kmk_thread_user_post' => array(array('user_id', false, false)),
		'kmk_thread_watch' => array(array('user_id', false, false)),
		'kmk_upgrade_log' => array(array('user_id', false, false)),
		'kmk_user_alert' => array(array('user_id', 'username')),
		'kmk_user_follow' => array(array('user_id', false, false), array('follow_user_id', false, false)),
		'kmk_user_ignored' => array(array('user_id', false, false), array('ignored_user_id', false, false)),
		'kmk_user_trophy' => array(array('user_id')),
		'kmk_user_upgrade_active' => array(array('user_id', false, false)),
		'kmk_user_upgrade_expired' => array(array('user_id', false, false)),
		'kmk_warning' => array(array('user_id'), array('warning_user_id')),
	);
	...
	public function mergeUsers(array $target, array $source)
	{
		...
		if ($dw->setExistingData($target))
		{
			...
			$this->_getDb()->query("
				UPDATE kmk_thread_user_post AS source, kmk_thread_user_post AS target
