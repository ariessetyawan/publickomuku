...
class XenForo_Model_Template extends XenForo_Model
{
	...
	public function getEffectiveTemplateListForStyle($styleId, array $conditions = array(), $fetchOptions = array())
	{
		...
		return $this->_getDb()->fetchAll($this->limitQueryResults('
			...
			FROM kmk_template_map AS template_map
			INNER JOIN kmk_template AS template ON
				(template_map.template_id = template.template_id)
			LEFT JOIN kmk_addon AS addon ON
	...
	public function getOutdatedTemplates()
	{
		return $this->fetchAllKeyed('
			...
			FROM kmk_template AS template
			INNER JOIN kmk_style AS style ON (style.style_id = template.style_id)
			INNER JOIN kmk_template_map AS map ON (map.style_id = style.parent_id AND map.title = template.title)
			INNER JOIN kmk_template AS parent ON (map.template_id = parent.template_id
				AND (
	...
	public function pruneEditHistory($cutOff = null)
	{
		...
		$db = $this->_getDb();
		return $db->delete('kmk_template_history', 'log_date < ' . $db->quote($cutOff));
	}
	...
	public function getParentMappedTemplateByTitle($title, $styleId)
	{
		...
		return $this->_getDb()->fetchRow('
			...
			FROM kmk_template_map AS template_map
			INNER JOIN kmk_style AS style ON
				(template_map.style_id = style.style_id)
			INNER JOIN kmk_template_map AS parent_template_map ON
	...
	public function deleteTemplatesForAddOn($addOnId)
	{
		...
		$db->query('
			DELETE FROM kmk_template_compiled
			WHERE style_id = 0
	...
	protected function _deleteTemplates(array $templateIds)
	{
		...
		$db->query('
			DELETE FROM kmk_template_include
			WHERE source_map_id IN (
				SELECT template_map_id
				FROM kmk_template AS template
				INNER JOIN kmk_template_map AS template_map ON
					(template.template_id = template_map.template_id)
				WHERE template.template_id IN ('. $quotedIds . ')
			)
		');
		$db->query('
			DELETE FROM kmk_template_phrase
		...
		$db->delete('kmk_template', "template_id IN ($quotedIds)");
		$db->delete('kmk_template_map', "template_id IN ($quotedIds)");
		$db->delete('kmk_template_modification_log', "template_id IN ($quotedIds)");
	}
	...
	public function compileAllTemplates($maxExecution = 0, $startStyle = 0, $startTemplate = 0)
	{
		...
		if ($complete)
		{
			$compiledRemove = $db->fetchAll("
				...
				FROM kmk_template_compiled AS c
				LEFT JOIN kmk_template_map AS m ON (c.title = m.title AND c.style_id = m.style_id)
				WHERE m.title IS NULL
			");
			foreach ($compiledRemove AS $remove)
			{
				$db->delete('kmk_template_compiled',
	...
	public function compileTemplateInStyleTree(array $parsedRecord)
	{
		...
		if ($includeDeleteIds)
		{
			$db->delete('kmk_template_include', 'source_map_id IN (' . $db->quote($includeDeleteIds) . ')');
		}
		if ($includeInserts)
		{
			$db->query("
				INSERT IGNORE INTO kmk_template_include
		...
		if ($phraseDeleteIds)
		{
			$db->delete('kmk_template_phrase', 'template_map_id IN (' . $db->quote($phraseDeleteIds) . ')');
		}
		if ($phraseInserts)
		{
			$db->query("
				INSERT IGNORE INTO kmk_template_phrase
	...
	public function compileAndInsertParsedTemplate($templateMapId, $parsedTemplate, $title, $compileStyleId, $doDbWrite = null)
	{
		...
		if ($doDbWrite)
		{
			$db->delete('kmk_template_include', 'source_map_id = ' . $db->quote($templateMapId));
			if ($ins)
			{
				$db->query("
					INSERT IGNORE INTO kmk_template_include
		...
		if ($doDbWrite)
		{
			$db->delete('kmk_template_phrase', 'template_map_id = ' . $db->quote($templateMapId));
			if ($ins)
			{
				$db->query("
					INSERT IGNORE INTO kmk_template_phrase
	...
	{
		$this->_getDb()->query("
			INSERT INTO kmk_template_compiled
	...
	public function insertTemplateMapForStyles(array $styleMapList, $truncate = false)
	{
		...
		if ($truncate)
		{
			$db->query('TRUNCATE TABLE kmk_template_map');
			$db->query('TRUNCATE TABLE kmk_template_include');
			$db->query('TRUNCATE TABLE kmk_template_phrase');
		}
		...
		foreach ($styleMapList AS $builtStyleId => $map)
		{
			if (!$truncate)
			{
				$db->delete('kmk_template_map', 'style_id = ' . $db->quote($builtStyleId));
			}

			foreach ($map AS $title => $templateId)
			{
				$db->insert('kmk_template_map', array(
	...
	public function writeTemplateFiles($enable = false, $handleOptions = true)
	{
		...
		$templates = $this->_getDb()->query('SELECT * FROM kmk_template_compiled');
