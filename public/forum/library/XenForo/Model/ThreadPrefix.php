...
class XenForo_Model_ThreadPrefix extends XenForo_Model
{
	...
	public function getPrefixes(array $conditions = array(), array $fetchOptions = array())
	{
		...
		$query = $this->limitQueryResults('
			...
			FROM kmk_thread_prefix AS prefix
		...
		if (!empty($fetchOptions['join']))
		{
			if ($fetchOptions['join'] & self::FETCH_FORUM_PREFIX)
			{
				$selectFields .= ',
					fp.*';
				$joinTables .= '
					INNER JOIN kmk_forum_prefix AS fp ON
			...
			if ($fetchOptions['join'] & self::FETCH_PREFIX_GROUP)
			{
				...
				$joinTables .= '
					LEFT JOIN kmk_thread_prefix_group AS prefix_group ON
	...
	public function getPrefixIfInForum($prefixId, $nodeId)
	{
		return $this->_getDb()->fetchRow('
			SELECT prefix.*
			FROM kmk_thread_prefix AS prefix
			INNER JOIN kmk_forum_prefix AS fp ON (fp.prefix_id = prefix.prefix_id AND fp.node_id = ?)
	...
	public function getVisiblePrefixIds(array $viewingUser = null)
	{
		...
		$results = $this->_getDb()->query("
			...
			FROM kmk_thread_prefix AS prefix
			INNER JOIN kmk_forum_prefix AS fp ON (fp.prefix_id = prefix.prefix_id)
			INNER JOIN kmk_node AS node ON (fp.node_id = node.node_id)
			INNER JOIN kmk_forum AS forum ON (node.node_id = forum.node_id)
	...
	public function updatePrefixForumAssociationByPrefix($prefixId, array $nodeIds)
	{
		...
		$db->delete('kmk_forum_prefix', 'prefix_id = ' . $db->quote($prefixId));

		foreach ($nodeIds AS $nodeId)
		{
			$db->insert('kmk_forum_prefix', array(
				'node_id' => $nodeId,
				'prefix_id' => $prefixId
			));
		}
	...
	public function updatePrefixForumAssociationByForum($nodeId, array $prefixIds)
	{
		...
		$db->delete('kmk_forum_prefix', 'node_id = ' . $db->quote($nodeId));
	...
	public function rebuildPrefixMaterializedOrder()
	{
		...
		{
			$db->query('
				UPDATE kmk_thread_prefix SET materialized_order = CASE prefix_id
				' . implode(' ', $updates) . '
				END
