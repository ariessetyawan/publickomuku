...
class XenForo_Model_AdminTemplate extends XenForo_Model
{
	...
	public function getAdminTemplateTitles(array $conditions = array(), array $fetchOptions = array())
	{
		...
		return $this->fetchAllKeyed($this->limitQueryResults('
			...
			FROM kmk_admin_template AS template
				LEFT JOIN kmk_addon AS addon ON (addon.addon_id = template.addon_id)
			WHERE ' . $whereClause . '
	...
	public function getDependentAdminTemplates($templateId)
	{
		return $this->_getDb()->fetchAll('
			...
			FROM kmk_admin_template AS admin_template
			INNER JOIN kmk_admin_template_include AS include ON
	...
	public function getIncludingTemplateIds($templateIds)
	{
		...
		return $db->fetchCol('
			SELECT source_id
			FROM kmk_admin_template_include
	...
	public function getAdminTemplatesThatIncludePhrase($phraseTitle)
	{
		return $this->fetchAllKeyed('
			SELECT admin_template.*
			FROM kmk_admin_template_phrase AS template_phrase
			INNER JOIN kmk_admin_template AS admin_template ON
	...
	public function deleteAdminTemplatesForAddOn($addOnId)
	{
		...
		if ($templateTitles)
		{
			...
			$db->delete('kmk_admin_template', "template_id IN ($quotedIds)");
			$db->delete('kmk_admin_template_compiled', 'title IN (' . $db->quote($templateTitles) . ')');
			$db->delete('kmk_admin_template_include', "source_id IN ($quotedIds)");
			$db->delete('kmk_admin_template_phrase', "template_id IN ($quotedIds)");
			$db->delete('kmk_admin_template_modification_log', "template_id IN ($quotedIds)");
		}
	...
	public function compileAllParsedAdminTemplates($maxExecution = 0, $startTemplate = 0, array $priority = array())
	{
		...
		if ($complete)
		{
			$compiledRemove = $db->fetchAll("
				...
				FROM kmk_admin_template_compiled AS c
				LEFT JOIN kmk_admin_template AS m ON (c.title = m.title)
				WHERE m.title IS NULL
			");
			foreach ($compiledRemove AS $remove)
			{
				$db->delete('kmk_admin_template_compiled',
	...
	public function compileParsedAdminTemplate($templateId, array $parsedTemplate, $title)
	{
		...
		if ($isCss)
		{
			...
			$db->query('
				INSERT INTO kmk_admin_template_compiled
		...
		$db->delete('kmk_admin_template_include', 'source_id = ' . $db->quote($templateId));
		if ($ins)
		{
			$db->query("
				INSERT IGNORE INTO kmk_admin_template_include
					(source_id, target_id)
				VALUES
					" . implode(',', $ins)
			);
		...
		$db->delete('kmk_admin_template_phrase', 'template_id = ' . $db->quote($templateId));
		if ($ins)
		{
			$db->query('
				INSERT IGNORE INTO kmk_admin_template_phrase
