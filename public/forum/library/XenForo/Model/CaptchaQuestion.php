...
class XenForo_Model_CaptchaQuestion extends XenForo_Model
{
	...
	{
		$this->_getDb()->delete('kmk_captcha_log', 'captcha_date < ' . (XenForo_Application::$time - $expiry));
	}
	...
	public function getTextCaptchaEntry()
	{
		...
		$this->_getDb()->insert('kmk_captcha_log', array(
			'hash' => $result['hash'],
	...
	public function verifyTextCaptcha($hash, $answer)
	{
		...
		$log = $db->fetchRow('SELECT * FROM kmk_captcha_log WHERE hash = ?', $hash);
		if ($log && $log['captcha_type'] == 'TextCaptcha')
		{
			if (!$db->delete('kmk_captcha_log', 'hash = ' . $db->quote($hash)))
			{
				return false;
			}
	...
	/**
	 * Inserts a hash into the kmk_captcha_log table,
	...
	public static function isCorrect($answer, $questionHash)
	{
		...
		if ($log = $db->fetchRow('SELECT * FROM kmk_captcha_log WHERE hash = ?', $questionHash))
		{
			if ($db->delete('kmk_captcha_log', 'hash = ' . $db->quote($questionHash)))
			{
