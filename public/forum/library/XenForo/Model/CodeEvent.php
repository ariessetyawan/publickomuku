...
class XenForo_Model_CodeEvent extends XenForo_Model
{
	...
	{
		$db = $this->_getDb();
		$db->delete('kmk_code_event', 'addon_id = ' . $db->quote($addOnId));
	}
	...
	public function getEventListenerArray()
	{
		...
		$listenerResult = $this->_getDb()->query('
			...
			FROM kmk_code_event_listener AS listener
	...
	{
		$db = $this->_getDb();
		$db->delete('kmk_code_event_listener', 'addon_id = ' . $db->quote($addOnId));
	}
