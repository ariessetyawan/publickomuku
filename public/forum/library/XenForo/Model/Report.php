...
class XenForo_Model_Report extends XenForo_Model
{
	...
	public function getReportById($id)
	{
		return $this->_getDb()->fetchRow('
			...
			FROM kmk_report AS report
			LEFT JOIN kmk_user AS assigned ON (assigned.user_id = report.assigned_user_id)
			LEFT JOIN kmk_user AS user ON (user.user_id = report.content_user_id)
			LEFT JOIN kmk_user_profile AS user_profile ON (user_profile.user_id = report.content_user_id)
			LEFT JOIN kmk_user_option AS user_option ON (user_option.user_id = report.content_user_id)
	...
	public function getReportComments($reportId, $orderDirection = 'ASC')
	{
		return $this->fetchAllKeyed("
			SELECT report_comment.*,
				user.*
			FROM kmk_report_comment AS report_comment
			LEFT JOIN kmk_user AS user ON (user.user_id = report_comment.user_id)
	...
	public function getReportingUsersForReport($reportId)
	{
		return $this->fetchAllKeyed("
			...
			FROM kmk_report_comment AS report_comment
			INNER JOIN kmk_user AS user ON (report_comment.user_id = user.user_id)
			WHERE report_comment.report_id = ?
