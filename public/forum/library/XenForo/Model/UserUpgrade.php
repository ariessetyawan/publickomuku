...
class XenForo_Model_UserUpgrade extends XenForo_Model
{
	...
	public function getUserUpgradeRecords(array $conditions = array(), array $fetchOptions = array())
	{
		...
		return $this->fetchAllKeyed($this->limitQueryResults(
			'
				...
				FROM kmk_' . $baseTable . ' AS ' . $baseTable . '
				LEFT JOIN kmk_user AS user ON (' . $baseTable . '.user_id = user.user_id)
		...
		if (!empty($fetchOptions['join']))
		{
			if ($fetchOptions['join'] & self::JOIN_UPGRADE)
			{
				$selectFields .= ',
					user_upgrade.*';
				$joinTables .= '
					LEFT JOIN kmk_user_upgrade AS user_upgrade ON
	...
	public function getActiveUserUpgradeRecordsForUser($userId)
	{
		return $this->fetchAllKeyed('
			SELECT user_upgrade_active.*,
				user_upgrade.*
			FROM kmk_user_upgrade_active AS user_upgrade_active
			LEFT JOIN kmk_user_upgrade AS user_upgrade ON (user_upgrade.user_upgrade_id = user_upgrade_active.user_upgrade_id)
	...
	public function getExpiredUserUpgradeRecordById($upgradeRecordId)
	{
		return $this->_getDb()->fetchRow('
			SELECT user_upgrade_expired.*,
				user.*
			FROM kmk_user_upgrade_expired AS user_upgrade_expired
			INNER JOIN kmk_user AS user ON
	...
	public function upgradeUser($userId, array $upgrade, $allowInsertUnpurchasable = false, $endDate = null)
	{
		...
		if ($active)
		{
			...
			{
				$db->update('kmk_user_upgrade_active',
		...
		else
		{
			...
			$db->insert('kmk_user_upgrade_active', array(
				'user_id' => $userId,
			...
			// there is a situation involving MySQL restarts where the auto increment value in kmk_user_upgrade_active
			// is lower than the highest value in kmk_user_upgrade_expired, so attempt to workaround that.
			$maxExpiredId = intval($db->fetchOne("
				SELECT MAX(user_upgrade_record_id)
				FROM kmk_user_upgrade_expired
			"));
			if ($upgradeRecordId <= $maxExpiredId)
			{
				$result = $db->query("
					UPDATE IGNORE kmk_user_upgrade_active
	...
	{
		$this->_getDb()->insert('kmk_user_upgrade_log', array(
	...
	public function getTransactionLogs(array $conditions = array(), array $fetchOptions = array())
	{
		...
		return $this->fetchAllKeyed($this->limitQueryResults(
			"
				...
				FROM kmk_user_upgrade_log AS log
				LEFT JOIN kmk_user_upgrade_active AS active ON (log.user_upgrade_record_id = active.user_upgrade_record_id)
				LEFT JOIN kmk_user_upgrade_expired AS expired ON (log.user_upgrade_record_id = expired.user_upgrade_record_id)
				LEFT JOIN kmk_user_upgrade AS user_upgrade ON (user_upgrade.user_upgrade_id = COALESCE(active.user_upgrade_id, expired.user_upgrade_id, 0))
				LEFT JOIN kmk_user AS user ON (user.user_id = COALESCE(active.user_id, expired.user_id, 0))
				WHERE " . $whereClause . "
	...
	public function downgradeUserUpgrades(array $upgrades, $sendAlert = true)
	{
		...
		$db->query('
			INSERT IGNORE INTO kmk_user_upgrade_expired
		...
		$db->delete('kmk_user_upgrade_active', 'user_upgrade_record_id IN (' . $db->quote($upgradeRecordIds) . ')');
	...
	{
		$db = $this->_getDb();
		$db->update('kmk_user_upgrade_active', array(
			'end_date' => $endDate
