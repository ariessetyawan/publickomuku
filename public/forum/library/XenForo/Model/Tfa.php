...
class XenForo_Model_Tfa extends XenForo_Model
{
	...
	public function enableUserTfaProvider($userId, $providerId, array $providerData)
	{
		...
		$success = $db->update('kmk_user_option', array('use_tfa' => 1), 'user_id = ' . $db->quote($userId));
	...
	public function disableTfaForUser($userId)
	{
		...
		$db->beginTransaction();
		$db->delete('kmk_user_tfa', 'user_id = ' . $db->quote($userId));
		$success = $db->update('kmk_user_option', array('use_tfa' => 0), 'user_id = ' . $db->quote($userId));
	...
	public function createTrustedKey($userId, $trustedUntil = null)
	{
		...
		$this->_getDb()->query("
			INSERT IGNORE INTO kmk_user_tfa_trusted
	...
	{
		$this->_getDb()->query("
			DELETE FROM kmk_user_tfa_trusted
			WHERE user_id = ?
				AND trusted_key = ?
	...
	public function pruneTrustedKeys($cutOff = null)
	{
		...
		$db = $this->_getDb();
		return $db->delete('kmk_user_tfa_trusted', 'trusted_until < ' . $db->quote($cutOff));
	}
	...
	{
		$this->_getDb()->insert('kmk_tfa_attempt', array(
			'user_id' => $userId,
	...
	{
		$this->_getDb()->delete('kmk_tfa_attempt', 'user_id = ' . $this->_getDb()->quote($userId));
	}
	...
	public function pruneFailedTfaAttempts($cutOff = null)
	{
		...
		$db = $this->_getDb();
		$db->delete('kmk_tfa_attempt', 'attempt_date < ' . $db->quote($cutOff));
	}
}