...
class XenForo_Model_RoutePrefix extends XenForo_Model
{
	...
	public function getPrefixesByRouteType($type, $getPrefixesForCache = false)
	{
		if ($getPrefixesForCache)
		{
			...
			$joinTables = 'LEFT JOIN kmk_addon AS addon ON (addon.addon_id = route_prefix.addon_id)';
		...
		return $this->fetchAllKeyed('
			SELECT route_prefix.*
			' . $selectFields . '
			FROM kmk_route_prefix AS route_prefix
			' . $joinTables . '
	...
	{
		$db = $this->_getDb();
		$db->delete('kmk_route_prefix', 'addon_id = ' . $db->quote($addOnId));
	}
