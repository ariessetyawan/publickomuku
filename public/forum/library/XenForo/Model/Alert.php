...
class XenForo_Model_Alert extends XenForo_Model
{
	...
	public function markAllAlertsReadForUser($userId, $time = null)
	{
		...
		$db->update('kmk_user_alert', array('view_date' => $time), $condition);
	...
	public function resetUnreadAlertsCounter($userId)
	{
		...
		{
			$db = $this->_getDb();
			$db->update('kmk_user', array('alerts_unread' => 0), 'user_id = ' . $db->quote($userId));
	...
	public function deleteOldReadAlerts($dateCut = null)
	{
		...
		$db = $this->_getDb();
		$db->delete('kmk_user_alert', 'view_date > 0 AND view_date < '. $db->quote($dateCut));
	}
	...
	public function deleteOldUnreadAlerts($dateCut = null)
	{
		...
		$db = $this->_getDb();
		$db->delete('kmk_user_alert', 'view_date = 0 AND event_date < '. $db->quote($dateCut));
	}
