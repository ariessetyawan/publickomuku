...
class XenForo_Model_Banning extends XenForo_Model
{
	...
	public function getBannedUsers(array $conditions = array(), array $fetchOptions = array())
	{
		...
		return $this->fetchAllKeyed($this->limitQueryResults(
			'
				...
				FROM kmk_user_ban AS user_ban
				LEFT JOIN kmk_user AS user ON (user.user_id = user_ban.user_id)
				LEFT JOIN kmk_user AS banning_user ON (banning_user.user_id = user_ban.ban_user_id)
	...
	public function banIp($ip)
	{
		...
		$result = $this->_getDb()->query('
			INSERT IGNORE INTO kmk_ip_match
	...
	public function deleteBannedIps(array $ips)
	{
		...
		$db->delete('kmk_ip_match', 'ip IN (' . $db->quote($ips) . ') AND match_type = ' . $db->quote('banned'));
		$this->rebuildBannedIpCache();
	...
	public function deleteDiscouragedIps(array $ips)
	{
		...
		$db->delete('kmk_ip_match', 'ip IN (' . $db->quote($ips) . ') AND match_type = ' . $db->quote('discouraged'));
		$this->rebuildDiscouragedIpCache();
	...
	{
		return $this->_getDb()->fetchOne('SELECT COUNT(*) FROM kmk_ban_email');
	}
	...
	public function banEmail($email)
	{
		...
		$result = $this->_getDb()->query('
			INSERT IGNORE INTO kmk_ban_email
				(banned_email)
			VALUES
				(?)
		', array($email));
	...
	public function deleteBannedEmails(array $emails)
	{
		...
		$db->delete('kmk_ban_email', 'banned_email IN (' . $db->quote($emails) . ')');
		$this->rebuildBannedEmailCache();
