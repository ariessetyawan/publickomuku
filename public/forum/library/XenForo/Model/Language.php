...
class XenForo_Model_Language extends XenForo_Model
{
	...
	*/
	...
	public function getAllLanguagesForCache()
	{
		...
		{
			$language['phrase_cache'] = XenForo_Helper_Php::safeUnserialize($language['phrase_cache']);
		}
	...
	{
		return $this->_getDb()->fetchOne('SELECT COUNT(*) FROM kmk_phrase WHERE language_id = 0');
	}
	...
	public function getLanguageXml(array $language, $limitAddOnId = null, $getUntranslated = false)
	{
		...
		if ($getUntranslated)
		{
			$phrases = $db->fetchAll('
				...
				FROM kmk_phrase_map AS map
				INNER JOIN kmk_phrase AS phrase ON (map.phrase_id = phrase.phrase_id)
				LEFT JOIN kmk_phrase AS master ON (master.title = phrase.title AND master.language_id = 0)
