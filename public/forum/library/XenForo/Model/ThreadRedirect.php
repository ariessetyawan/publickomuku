...
class XenForo_Model_ThreadRedirect extends XenForo_Model
{
	...
	{
		$this->_getDb()->insert('kmk_thread_redirect', array(
			'thread_id' => $threadId,
	...
	public function deleteThreadRedirects(array $threadIds)
	{
		...
		$db->delete('kmk_thread_redirect', 'thread_id IN (' . $db->quote($threadIds) . ')');
	...
	{
		$db = $this->_getDb();
		$db->update('kmk_thread_redirect', $update, 'thread_id = ' . $db->quote($threadId));
	}
}