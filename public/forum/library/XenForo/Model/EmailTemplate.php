...
class XenForo_Model_EmailTemplate extends XenForo_Model
{
	...
	public function getAllEffectiveEmailTemplateTitles()
	{
		return $this->fetchAllKeyed('
			...
			FROM kmk_email_template AS master
			LEFT JOIN kmk_email_template AS custom ON
	...
	public function getAllMasterEmailTemplateTitles()
	{
		return $this->fetchAllKeyed('
			...
			FROM kmk_email_template AS email_template
			LEFT JOIN kmk_addon AS addon ON
	...
	public function getEffectiveEmailTemplatesThatIncludePhrase($phraseTitle)
	{
		return $this->fetchAllKeyed('
			...
			FROM kmk_email_template_phrase AS template_phrase
			INNER JOIN kmk_email_template AS master ON
	...
	public function compileAllEmailTemplates()
	{
		...
		$compiledRemove = $db->fetchAll("
			...
			FROM kmk_email_template_compiled AS c
			LEFT JOIN kmk_email_template AS m ON (c.title = m.title)
			WHERE m.title IS NULL
		");
		foreach ($compiledRemove AS $remove)
		{
			$db->delete('kmk_email_template_compiled',
	...
	public function compileAndInsertEmailTemplate(array $template)
	{
		...
		foreach ($languages AS $language)
		{
			...
			$db->query('
				INSERT INTO kmk_email_template_compiled
		...
		$db->delete('kmk_email_template_phrase', 'title = ' . $db->quote($title));
		foreach ($phrases AS $includedPhrase)
		{
			...
			$db->insert('kmk_email_template_phrase', array(
				'title' => $title,
	...
	public function deleteEmailTemplatesForAddOn($addOnId)
	{
		...
		if ($titles)
		{
			...
			$db->delete('kmk_email_template', "title IN ($quotedTitles) AND custom = 0");
			$db->delete('kmk_email_template_compiled', "title IN ($quotedTitles)");
			$db->delete('kmk_email_template_phrase', "title IN ($quotedTitles)");
			$db->delete('kmk_email_template_modification_log', "template_id IN ($quotedIds)");
		}
