...
class XenForo_Model_StyleProperty extends XenForo_Model
{
	...
	public function getStylePropertyById($id)
	{
		return $this->_getDb()->fetchRow('
			SELECT property_definition.*,
				style_property.*
			FROM kmk_style_property AS style_property
			INNER JOIN kmk_style_property_definition AS property_definition ON
	...
	public function getStylePropertyDefinitionById($propertyDefinitionId)
	{
		return $this->_getDb()->fetchRow('
			...
			FROM kmk_style_property_definition AS property_definition
			LEFT JOIN kmk_style_property AS property ON
	...
	public function getStylePropertyDefinitionsForAdminQuickSearch($searchText, array $propertyNames)
	{
		...
		return $this->fetchAllKeyed('
			SELECT *
			FROM kmk_style_property_definition
			WHERE ' . $sqlConditions,
		'property_name');
	}
	...
	public function deleteStylePropertiesAndDefinitionsInStyle($styleId, $addOnId = null, $leaveChildCustomizations = false)
	{
		...
		if ($delPropertyIds)
		{
			$this->_db->delete('kmk_style_property',
		...
		{
			$this->_db->delete('kmk_style_property_definition',
	...
	public function deleteStylePropertyGroupsInStyle($styleId, $addOnId = null)
	{
		...
		if ($addOnId === null)
		{
			$db->delete('kmk_style_property_group', 'group_style_id = ' . $db->quote($styleId));
		}
		else
		{
			$db->delete('kmk_style_property_group',
		...
		foreach ($newProperties AS $property)
		{
			...
			if (!empty($property['definition']))
			{
				if (!$definition)
				{
					...
					{
						$db->update('kmk_style_property', array(
