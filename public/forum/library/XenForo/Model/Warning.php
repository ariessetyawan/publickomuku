...
class XenForo_Model_Warning extends XenForo_Model
{
	...
	public function getWarningById($id)
	{
		return $this->_getDb()->fetchRow('
			...
			FROM kmk_warning AS warning
			LEFT JOIN kmk_user AS user ON (user.user_id = warning.user_id)
			LEFT JOIN kmk_user AS warn_user ON (warn_user.user_id = warning.warning_user_id)
	...
	public function triggerWarningAction($userId, array $action)
	{
		...
		if ($insertTrigger)
		{
			$this->_getDb()->insert('kmk_warning_action_trigger', array(
	...
	public function removeWarningActionTrigger($userId, array $trigger)
	{
		...
		$db->delete('kmk_warning_action_trigger', 'action_trigger_id = ' . $db->quote($trigger['action_trigger_id']));
