...
class XenForo_Model_Phrase extends XenForo_Model
{
	...
	)
	{
		...
		return $this->_getDb()->fetchAll($this->limitQueryResults(
			'
				...
				FROM kmk_phrase_map AS phrase_map
	...
	public function getOutdatedPhrases()
	{
		return $this->fetchAllKeyed('
			...
			FROM kmk_phrase AS phrase
			INNER JOIN kmk_phrase AS master ON (master.title = phrase.title AND master.language_id = 0)
			INNER JOIN kmk_language AS language ON (language.language_id = phrase.language_id)
	...
	public function getParentMappedPhraseByTitle($title, $languageId)
	{
		...
		return $this->_getDb()->fetchRow('
			...
			FROM kmk_phrase_map AS phrase_map
			INNER JOIN kmk_language AS language ON
				(phrase_map.language_id = language.language_id)
			INNER JOIN kmk_phrase_map AS parent_phrase_map ON
	...
	public function compileAllPhrases($maxExecution = 0, $startLanguage = 0, $startPhrase = 0)
	{
		...
		if ($complete)
		{
			$compiledRemove = $db->fetchAll("
				...
				FROM kmk_phrase_compiled AS c
				LEFT JOIN kmk_phrase_map AS m ON (c.title = m.title AND c.language_id = m.language_id)
				WHERE m.title IS NULL
			");
			foreach ($compiledRemove AS $remove)
			{
				$db->delete('kmk_phrase_compiled',
	...
	public function compileAndInsertParsedPhrase($phraseMapId, $parsedPhrase, $title, $compileLanguageId)
	{
		...
		$db->query('
			INSERT INTO kmk_phrase_compiled
	...
	public function insertPhraseMapForLanguages(array $languageMapList, $truncate = false)
	{
		...
		if ($truncate)
		{
			$db->query('TRUNCATE TABLE kmk_phrase_map');
		}
		...
		foreach ($languageMapList AS $builtLanguageId => $map)
		{
			if (!$truncate)
			{
				$db->delete('kmk_phrase_map', 'language_id = ' . $db->quote($builtLanguageId));
			}
			...
			{
				/*$db->insert('kmk_phrase_map', array(
	...
	public function deletePhrasesForAddOn($addOnId)
	{
		...
		', $addOnId);
		$db->query('
			DELETE FROM kmk_phrase_compiled
			WHERE language_id = 0
				AND title IN (
					SELECT title
					FROM kmk_phrase
		...
		', $addOnId);
		$db->delete('kmk_phrase', 'language_id = 0 AND addon_id = ' . $db->quote($addOnId));
	}
	...
	public function mapUnhandledPhrases()
	{
		$phrases = $this->_getDb()->fetchAll('
			SELECT phrase.*
			FROM kmk_phrase AS phrase
			LEFT JOIN kmk_phrase_map AS pm ON
