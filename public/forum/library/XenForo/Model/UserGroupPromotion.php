...
class XenForo_Model_UserGroupPromotion extends XenForo_Model
{
	...
	public function getPromotions(array $conditions = array())
	{
		...
		return $this->fetchAllKeyed('
			SELECT promotion.*
			FROM kmk_user_group_promotion AS promotion
			WHERE ' . $whereClause . '
	...
	public function getPromotionStatesByUserId($userId)
	{
		$results = $this->_getDb()->query('
			...
			FROM kmk_user_group_promotion_log
			WHERE user_id = ?
		', $userId);
	...
	public function getPromotionLogEntries(array $conditions = array(), array $fetchOptions = array())
	{
		...
		return $this->_getDb()->fetchAll($this->limitQueryResults(
			'
				...
				FROM kmk_user_group_promotion_log AS log
		...
		if (!empty($fetchOptions['join']))
		{
			if ($fetchOptions['join'] & self::FETCH_USER_NAME)
			{
				$selectFields .= ',
					user.username';
				$joinTables .= '
					INNER JOIN kmk_user AS user ON (log.user_id = user.user_id)';
			}
			...
			if ($fetchOptions['join'] & self::FETCH_PROMOTION_TITLE)
			{
				$selectFields .= ',
					promotion.title';
				$joinTables .= '
					INNER JOIN kmk_user_group_promotion AS promotion ON (log.promotion_id = promotion.promotion_id)';
			}
		}
	...
	public function demoteUser(array $promotion, $userId, $disablePromotion = false)
	{
		...
		else
		{
			...
			$db->delete('kmk_user_group_promotion_log',
	...
	{
		$this->_getDb()->query('
			INSERT INTO kmk_user_group_promotion_log
