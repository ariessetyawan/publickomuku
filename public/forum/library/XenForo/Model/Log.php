...
class XenForo_Model_Log extends XenForo_Model
{
	public function getServerErrorLogById($id)
	{
		return $this->_getDb()->fetchRow('
			...
			FROM kmk_error_log AS error_log
			LEFT JOIN kmk_user AS user ON (user.user_id = error_log.user_id)
	...
	{
		$db = $this->_getDb();
		$db->delete('kmk_error_log', 'error_id = ' . $db->quote($id));
	}

	public function clearServerErrorLog()
	{
		$this->_getDb()->query('TRUNCATE TABLE kmk_error_log');
	}
	...
	public function logAdminRequest(Zend_Controller_Request_Http $request, array $requestData = null, $ipAddress = null)
	{
		...
		$this->_getDb()->insert('kmk_admin_log', array(
	...
	public function getAdminLogEntries($userId = 0, array $fetchOptions = array())
	{
		...
		return $this->fetchAllKeyed($this->limitQueryResults(
			'
				...
				FROM kmk_admin_log AS admin_log
				INNER JOIN kmk_user AS user ON (user.user_id = admin_log.user_id)
	...
	public function getModeratorLogForDiscussionContent($discussionType, $discussionId)
	{
		return $this->fetchAllKeyed('
			...
			FROM kmk_moderator_log AS moderator_log
			INNER JOIN kmk_user AS user ON (user.user_id = moderator_log.user_id)
