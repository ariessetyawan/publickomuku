...
class XenForo_Model_Tag extends XenForo_Model
{
	...
	public function getTagsForContent($contentType, $contentId)
	{
		return $this->fetchAllKeyed("
			...
			FROM kmk_tag_content AS tag_content
			INNER JOIN kmk_tag AS tag ON (tag.tag_id = tag_content.tag_id)
	...
	public function recalculateTagUsage($tagIds)
	{
		...
		foreach ($tags AS $tagId => $permanent)
		{
			...
			if ($delete)
			{
				...
				{
					$db->delete('kmk_tag', 'tag_id = ' . $db->quote($tagId));
				}
			}
		}
	...
	public function mergeTags($sourceTagId, $targetTagId)
	{
		...
		$db->query("
			UPDATE IGNORE kmk_tag_content
			SET tag_id = ?
			WHERE tag_id = ?
		", array($targetTagId, $sourceTagId));
		$db->query("DELETE FROM kmk_tag_result_cache WHERE tag_id = ?", $targetTagId);

		// this handles cases where the content already had the target tag
		$db->query("DELETE FROM kmk_tag WHERE tag_id = ?", $sourceTagId);
		$db->query("DELETE FROM kmk_tag_content WHERE tag_id = ?", $sourceTagId);
	...
	public function insertTagResultsCache($tagId, array $results, $userId = null)
	{
		...
		$this->_getDb()->query("
			INSERT INTO kmk_tag_result_cache
	...
	public function pruneTagResultsCache($cutOff = null)
	{
		...
		$this->_getDb()->delete('kmk_tag_result_cache', 'expiry_date <= ' . intval($cutOff));
	}
