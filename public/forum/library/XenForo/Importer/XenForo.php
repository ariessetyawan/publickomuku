...
class XenForo_Importer_XenForo extends XenForo_Importer_Abstract
{
	...
	public function validateConfiguration(array &$config)
	{
		...
		try
		{
			$db->query('SELECT user_id FROM kmk_user LIMIT 1');
		}
		catch (Zend_Db_Exception $e)
		{
			if ($config['db']['dbname'] === '')
	...
	{
		$groups = $this->_sourceDb->fetchAll('SELECT * FROM kmk_user_group ORDER BY user_group_id');
	...
	public function stepUserFields($start, array $options)
	{
		...
		$userFields = $this->_sourceDb->fetchAll("
			...
			FROM kmk_user_field AS field
			INNER JOIN kmk_phrase AS ptitle ON
			(
				ptitle.language_id = 0 AND
				ptitle.title = CONCAT('user_field_', field.field_id)
			)
			INNER JOIN kmk_phrase AS pdesc ON
			(
	...
	public function stepUsers($start, array $options)
	{
		...
		{
			$options['max'] = $sDb->fetchOne('SELECT MAX(user_id) FROM kmk_user');
		}

		$users = $sDb->fetchAll(
			$sDb->limit($this->_getSelectUserSql('kmk_user.user_id > ' . $sDb->quote($start)), $options['limit'])
		);
		if (!$users)
		{
	...
	public function stepUsersMerge($start, array $options)
	{
		...
		if ($manual)
		{
			$merge = $sDb->fetchAll($this->_getSelectUserSql('kmk_user.user_id IN (' . $sDb->quote(array_keys($manual)) . ')'));
	...
	public function stepUsersFailed($start, array $options)
	{
		...
		if ($manual)
		{
			$users = $this->_sourceDb->fetchAll($this->_getSelectUserSql('kmk_user.user_id IN (' . $sDb->quote(array_keys($manual)) . ')'));
	...
	protected function _getSelectUserSql($where)
	{
		return '
			SELECT
				...
				kmk_user_privacy.*,
				kmk_user_profile.*,
				kmk_user_authenticate.*
			FROM kmk_user
			INNER JOIN  kmk_user_option ON
				(kmk_user_option.user_id = kmk_user.user_id)
			INNER JOIN  kmk_user_privacy ON
				(kmk_user_privacy.user_id = kmk_user.user_id)
			INNER JOIN  kmk_user_profile ON
				(kmk_user_profile.user_id = kmk_user.user_id)
			INNER JOIN kmk_user_authenticate ON
				(kmk_user_authenticate.user_id = kmk_user.user_id)
			WHERE '  . $where . '
			ORDER BY kmk_user.user_id
		';
	}
	...
	protected function _importUser(array $user, array $options = array())
	{
		...
		if ($importedUserId)
		{
			...
			if ($user['is_banned'])
			{
				if ($ban = $this->_sourceDb->fetchRow('SELECT * FROM kmk_user_ban WHERE user_id = ?', $user['user_id']))
				{
			...
			{
				if ($admin = $this->_sourceDb->fetchRow('SELECT * FROM kmk_admin WHERE user_id = ?', $user['user_id']))
				{
	...
	/**
	 * Translates kmk_user.secondary_group_ids to new group IDs
	 *
	...
	public function stepFollowIgnore($start, array $options)
	{
		...
		$follows = $sDb->fetchAll('SELECT * FROM kmk_user_follow WHERE user_id IN(' . $sDb->quote($searchUserIds) . ')');
		...
		$ignores = $sDb->fetchAll('SELECT * FROM kmk_user_ignored WHERE user_id IN(' . $sDb->quote($searchUserIds) . ')');
	...
	public function stepConversations($start, array $options)
	{
		...
		foreach ($conversations AS $conversation)
		{
			...
			$recipientRecords = $sDb->fetchAll('
				SELECT *
				FROM kmk_conversation_recipient
	...
	public function stepProfilePosts($start, array $options)
	{
		...
		if ($options['max'] === false)
		{
			$options['max'] = $sDb->fetchOne('
				...
				FROM kmk_profile_post
			');
		}

		$profilePosts = $sDb->fetchAll($sDb->limit(
			'
				SELECT kmk_profile_post.*, kmk_ip.ip
				FROM kmk_profile_post
				LEFT JOIN kmk_ip ON (kmk_ip.ip_id = kmk_profile_post.ip_id)
				WHERE kmk_profile_post.profile_post_id > ' . $sDb->quote($start) . '
				ORDER BY kmk_profile_post.profile_post_id
			', $options['limit']
		));
		if (!$profilePosts)
		{
		...
		foreach ($profilePosts AS $profilePost)
		{
			...
			if ($profilePost['comment_count'])
			{
				...
				$comments = $sDb->fetchAll('
					SELECT kmk_profile_post_comment.*, kmk_ip.ip
					FROM kmk_profile_post_comment
					LEFT JOIN kmk_ip ON (kmk_ip.ip_id = kmk_profile_post_comment.ip_id)
					WHERE kmk_profile_post_comment.profile_post_id = ?
			...
			if ($profilePostId = $model->importProfilePost($profilePost['profile_post_id'], $importProfilePost))
			{
				if (!empty($comments))
				{
					...
					$db->update('kmk_profile_post', array(
	...
	public function stepNodes($start, array $options)
	{
		...
		$nodeData = array();
		foreach ($sDb->fetchAll('SELECT * FROM kmk_forum') AS $forum)
		{
			$nodeData[$forum['node_id']] = $forum;
		}
		foreach ($sDb->fetchAll('SELECT * FROM kmk_page') AS $page)
		{
			$nodeData[$page['node_id']] = $page;
		}
		foreach ($sDb->fetchAll('SELECT * FROM kmk_link_forum') AS $linkForum)
		{
		...
		foreach ($sDb->fetchAll("SELECT * FROM kmk_permission_entry_content WHERE content_type = 'node'") AS $nodePermission)
		{
	...
	public function stepModerators($start, array $options)
	{
		...
		$moderators = $sDb->fetchAll('SELECT * FROM kmk_moderator');
		if (!$moderators)
		{
			return true;
		}

		$nodeModerators = $sDb->fetchAll("SELECT * FROM kmk_moderator_content WHERE content_type = 'node'");
	...
	public function stepThreadPrefixes($start, array $options)
	{
		...
		$prefixes = $sDb->fetchAll("
			SELECT tp.*,
				p.phrase_text
			FROM kmk_thread_prefix AS tp
			INNER JOIN kmk_phrase AS p
			ON (
				p.language_id = 0 AND
		...
		$prefixGroups = $sDb->fetchAll("
			SELECT tpg.*,
				p.phrase_text
			FROM kmk_thread_prefix_group AS tpg
			INNER JOIN kmk_phrase AS p
			ON (
				p.language_id = 0 AND
		...
		$forumPrefixes = array();
		foreach ($sDb->fetchAll('SELECT * FROM kmk_forum_prefix') AS $forumPrefix)
		{
	...
	public function stepThreads($start, array $options)
	{
		...
		$threads = $sDb->fetchAll($sDb->limit(
			"
				...
				LEFT JOIN kmk_user AS user ON (user.user_id = thread.user_id)
		...
		foreach ($threads AS $thread)
		{
			...
			$posts = $sDb->fetchAll($sDb->limit(
				"
					...
					LEFT JOIN kmk_user AS user ON (user.user_id = post.user_id)
					LEFT JOIN kmk_ip ON (kmk_ip.ip_id = post.ip_id)
			...
			if ($threadId)
			{
				...
				foreach ($posts AS $post)
				{
					if (!empty($post['quotes']))
					{
						...
						{
							$db->update('kmk_post', array('message' => $postQuotesRewrite), 'post_id = ' . $db->quote($post['new_post_id']));
						}
					}
				}
			}
	...
	public function stepContentTags($start, array $options)
	{
		...
		$tags = $sDb->fetchAll($sDb->limit(
			'
				SELECT content.*, tag.*
					FROM kmk_tag_content AS content
				INNER JOIN kmk_tag AS tag ON
	...
	public function stepAttachments($start, array $options)
	{
		...
		$attachments = $sDb->fetchAll($sDb->limit(
			'
				SELECT *
				FROM kmk_attachment AS a
				LEFT JOIN kmk_attachment_data AS ad ON (ad.data_id = a.data_id)
	...
	public function stepWarnings($start, array $options)
	{
		...
		/*if ($start == 0)
		{
			...
			$warningDefinitions = $sDb->fetchAll("
				SELECT wd.*,
					phrase.phrase_text
				FROM kmk_warning_definition AS wd
				INNER JOIN kmk_phrase AS phrase ON
				(
	...
	public function stepUserUpgrades($start, array $options)
	{
		...
		$activeUpgrades = $sDb->fetchAll($sDb->limit(
			'
				SELECT uua.*,
					uu.extra_group_ids
				FROM kmk_user_upgrade_active AS uua
				INNER JOIN kmk_user_upgrade AS uu ON
