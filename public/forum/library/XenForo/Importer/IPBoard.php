	...
	public function stepStatusUpdates($start, array $options)
	{
		...
		foreach ($statusUpdates AS $statusUpdate)
		{
			...
			if ($profilePostId = $model->importProfilePost($statusUpdate['status_id'], $import))
			{
				...
				{
					$db->update('kmk_user_profile', array
					(
				...
				if ($importUpdate)
				{
					$db->update('kmk_profile_post', $importUpdate, 'profile_post_id = ' . $sDb->quote($profilePostId));
				}
			}
		}
		...
		foreach ($threads AS $thread)
		{
			...
			if ($threadId)
			{
				...
				foreach ($posts AS $i => $post)
				{
					...
					$post['kmk_post_id'] = $model->importPost($post['pid'], $import);
				...
				foreach ($posts AS $post)
				{
					if (!empty($post['quotes']))
					{
						...
						{
							$db->update('kmk_post', array('message' => $postQuotesRewrite), 'post_id = ' . $db->quote($post['kmk_post_id']));
						}
					}
				}
			}
