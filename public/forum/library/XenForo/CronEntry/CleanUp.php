...
class XenForo_CronEntry_CleanUp
{
	...
	public static function runDailyCleanUp()
	{
		...
		$db->delete('kmk_thread_read', 'thread_read_date < ' . $readMarkingCutOff);
		$db->delete('kmk_forum_read', 'forum_read_date < ' . $readMarkingCutOff);

		// delete old searches
		$db->delete('kmk_search', 'search_date < ' . (XenForo_Application::$time - 86400));
