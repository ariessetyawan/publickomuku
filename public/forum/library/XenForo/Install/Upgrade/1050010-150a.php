...
class XenForo_Install_Upgrade_1050010 extends XenForo_Install_Upgrade_Abstract
{
	...
	public function step5()
	{
		...
		$this->executeUpgradeQuery("
			INSERT IGNORE INTO `kmk_tfa_provider`
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE `kmk_user_option` ADD `use_tfa` TINYINT UNSIGNED NOT NULL DEFAULT '0'
		");
	}
	...
	public function step9()
	{
		...
		$this->executeUpgradeQuery("
			ALTER TABLE  `kmk_forum` ADD  `min_tags` SMALLINT UNSIGNED NOT NULL DEFAULT  '0'
		");

		$this->executeUpgradeQuery("
			INSERT IGNORE INTO kmk_admin_permission_entry
				(user_id, admin_permission_id)
			SELECT user_id, 'tag'
			FROM kmk_admin_permission_entry
			WHERE admin_permission_id = 'thread'
		");

		$this->applyGlobalPermission('general', 'createTag', 'forum', 'postThread', false);

		$this->applyGlobalPermission('forum', 'tagOwnThread', 'forum', 'postThread', false);
		$this->applyGlobalPermission('forum', 'manageOthersTagsOwnThread', 'forum', 'deleteOwnThread', false);
		$this->applyGlobalPermission('forum', 'manageAnyTag', 'forum', 'manageAnyThread', true);

		$this->applyContentPermission('forum', 'tagOwnThread', 'forum', 'postThread', false);
		$this->applyContentPermission('forum', 'manageOthersTagsOwnThread', 'forum', 'deleteOwnThread', false);
		$this->applyContentPermission('forum', 'manageAnyTag', 'forum', 'manageAnyThread', true);
	}
