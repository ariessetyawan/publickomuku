...
class XenForo_Install_Upgrade_1030010 extends XenForo_Install_Upgrade_Abstract
{
	...
	public function step1()
	{
		...
		$this->executeUpgradeQuery($tables['kmk_image_proxy']);
		$this->executeUpgradeQuery($tables['kmk_link_proxy']);
		$this->executeUpgradeQuery($tables['kmk_smilie_category']);
		$this->executeUpgradeQuery($tables['kmk_bb_code']);
		$this->executeUpgradeQuery($tables['kmk_user_change_log']);
		$this->executeUpgradeQuery($tables['kmk_user_change_temp']);
		$this->executeUpgradeQuery($tables['kmk_spam_trigger_log']);

		// the table storage is different - just replace it
		$this->executeUpgradeQuery("DROP TABLE kmk_registration_spam_cache");
		...
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_user_field ADD moderator_editable TINYINT UNSIGNED NOT NULL DEFAULT '0'
		");

		$this->executeUpgradeQuery("
			ALTER TABLE kmk_phrase CHANGE title title VARBINARY(100) NOT NULL
		");

		$this->executeUpgradeQuery("
			ALTER TABLE kmk_session_activity
		...
		");
		$this->executeUpgradeQuery("UPDATE kmk_smilie SET display_order = smilie_id");
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_admin_log
				CHANGE ip_address ip_address_old INT UNSIGNED NOT NULL DEFAULT 0,
				ADD ip_address VARBINARY(16) NOT NULL DEFAULT ''
		");
		$this->executeUpgradeQuery("
			UPDATE kmk_admin_log SET ip_address = UNHEX(LPAD(HEX(ip_address_old), 8, '0'))
		");
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_admin_log DROP ip_address_old
		");
	}
	...
	public function step4()
	{
		...
		");
		$this->executeUpgradeQuery("
			UPDATE kmk_moderator_log SET ip_address = UNHEX(LPAD(HEX(ip_address_old), 8, '0'))
		");
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_moderator_log DROP ip_address_old
		");
	}
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_error_log
				CHANGE ip_address ip_address_old INT UNSIGNED NOT NULL DEFAULT 0,
				ADD ip_address VARBINARY(16) NOT NULL DEFAULT ''
		");
		$this->executeUpgradeQuery("
			UPDATE kmk_error_log SET ip_address = UNHEX(LPAD(HEX(ip_address_old), 8, '0'))
		");
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_error_log DROP ip_address_old
		");
	}
	...
	public function step6()
	{
		...
		");
		$this->executeUpgradeQuery("
			UPDATE kmk_login_attempt SET ip_address = UNHEX(LPAD(HEX(ip_address_old), 8, '0'))
		");
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_login_attempt
				DROP ip_address_old,
	...
	{
		$this->executeUpgradeQuery("
			UPDATE kmk_ip SET ip = UNHEX(LPAD(HEX(ip_old), 8, '0'))
		");
	}
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE `kmk_warning_action`
		...
		");
		$this->executeUpgradeQuery("
			ALTER TABLE  `kmk_warning_action_trigger` CHANGE  `action`  `action` VARBINARY( 25 ) NOT NULL
		");
	}
	...
	{
		$this->executeUpgradeQuery("
			UPDATE kmk_warning_action_trigger
			SET action = 'ban'
			WHERE action = 'ban_points'
		");
		$this->executeUpgradeQuery("
			INSERT IGNORE INTO kmk_user_change_temp
			...
			FROM kmk_warning_action_trigger
	...
	public function step14($position, array $stepData)
	{
		...
		{
			$stepData['max'] = $db->fetchOne('SELECT MAX(user_id) FROM kmk_user_external_auth');
		}

		$userIds = $db->fetchCol($db->limit(
			'
				SELECT DISTINCT user_id
				FROM kmk_user_external_auth AS user
				WHERE user_id > ?
				ORDER BY user_id
			', $perPage
		), $position);
	...
	public function step15()
	{
		...
		$values = $db->fetchOne("SELECT option_value FROM kmk_option WHERE option_id = 'censorWords'");
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_conversation_message
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_conversation_recipient
				ADD INDEX user_id (user_id)
		");
	}
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_attachment_data
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_profile_post_comment
				ADD INDEX user_id (user_id),
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_report_comment
				ADD INDEX user_id (user_id)
		");
	}
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_notice_dismissed
				ADD INDEX user_id (user_id)
		");
	}
