...
class XenForo_Install_Upgrade_1010031 extends XenForo_Install_Upgrade_Abstract
{
	...
	public function step1()
	{
		...
		$this->executeUpgradeQuery($tables['kmk_stats_daily']);
		$this->executeUpgradeQuery($tables['kmk_admin_log']);
		$this->executeUpgradeQuery($tables['kmk_moderator_log']);

		// admin search
		$this->executeUpgradeQuery($tables['kmk_admin_search_type']);
		$this->executeUpgradeQuery($data['kmk_admin_search_type']);

		// misc
		$this->executeUpgradeQuery("ALTER TABLE kmk_search ADD user_results MEDIUMBLOB NOT NULL AFTER search_grouping");
		$this->executeUpgradeQuery("ALTER TABLE kmk_language ADD text_direction enum('LTR','RTL') NOT NULL DEFAULT 'LTR'");
		$this->executeUpgradeQuery("ALTER TABLE kmk_trophy CHANGE criteria user_criteria MEDIUMBLOB NOT NULL");
		...
		$this->executeUpgradeQuery($tables['kmk_user_group_promotion']);
		$this->executeUpgradeQuery($tables['kmk_user_group_promotion_log']);

		// notices
		$this->executeUpgradeQuery($tables['kmk_notice']);
		$this->executeUpgradeQuery($tables['kmk_notice_dismissed']);

		// custom user fields and ignore list
		$this->executeUpgradeQuery($tables['kmk_user_field']);
		$this->executeUpgradeQuery($tables['kmk_user_field_value']);
		$this->executeUpgradeQuery($tables['kmk_user_ignored']);
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_user_profile
				ADD custom_fields MEDIUMBLOB NOT NULL,
				ADD ignored TEXT NOT NULL COMMENT 'Comma-separated integers from kmk_user_ignored'
		");

		// conversation attachments
		$this->executeUpgradeQuery("ALTER TABLE kmk_conversation_message ADD attach_count SMALLINT UNSIGNED NOT NULL DEFAULT 0");
		...
		// basic warning stuff
		$this->executeUpgradeQuery($tables['kmk_warning']);
		$this->executeUpgradeQuery($tables['kmk_warning_definition']);
		$this->executeUpgradeQuery($tables['kmk_warning_action']);
		$this->executeUpgradeQuery($tables['kmk_warning_action_trigger']);
		...
		// default warnings
		$db->query("
			INSERT IGNORE INTO kmk_warning_definition
	...
	public function step2()
	{
		...
		$this->executeUpgradeQuery($tables['kmk_forum_prefix']);
		$this->executeUpgradeQuery($tables['kmk_thread_prefix']);
		$this->executeUpgradeQuery($tables['kmk_thread_prefix_group']);
		...
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_forum
				...
				ADD prefix_cache MEDIUMBLOB NOT NULL COMMENT 'Serialized data from kmk_forum_prefix, [group_id][prefix_id] => prefix_id',
				ADD default_prefix_id INT UNSIGNED NOT NULL DEFAULT 0
		");

		$this->executeUpgradeQuery("ALTER TABLE kmk_thread ADD prefix_id INT UNSIGNED NOT NULL DEFAULT 0");
	...
	public function step3()
	{
		...
		$this->executeUpgradeQuery('ALTER TABLE kmk_user ADD warning_points INT UNSIGNED NOT NULL DEFAULT 0');

		$this->executeUpgradeQuery('
			ALTER TABLE kmk_profile_post
	...
	public function step5()
	{
		...
		if (!$this->executeUpgradeQuery('SELECT 1 FROM kmk_identity_service LIMIT 1'))
		{
		...
		$identities = $db->fetchAll("
			...
			FROM kmk_identity_service AS ident
			LEFT JOIN kmk_phrase AS title ON
				(title.language_id = 0 AND title.title = CONCAT('identity_service_name_', ident.identity_service_id))
			LEFT JOIN kmk_phrase AS hint ON
	...
	public function step6($position, array $stepData)
	{
		...
		{
			$stepData['max'] = $db->fetchOne('SELECT MAX(user_id) FROM kmk_user');
		}
	...
	public function step7()
	{
		...
		$this->executeUpgradeQuery('DROP TABLE kmk_identity_service');
		$this->executeUpgradeQuery('DROP TABLE kmk_user_identity');
		$this->executeUpgradeQuery('ALTER TABLE kmk_user_profile DROP identities');
		$this->executeUpgradeQuery("DELETE FROM kmk_phrase WHERE title LIKE 'identity_service_%'");

		// switch ident service admin perm to custom user fields
		$db->query("
			UPDATE IGNORE kmk_admin_permission_entry SET
