...
class XenForo_Install_Upgrade_1040010 extends XenForo_Install_Upgrade_Abstract
{
	...
	public function step1()
	{
		...
		$this->executeUpgradeQuery($tables['kmk_email_bounce_log']);
		$this->executeUpgradeQuery($tables['kmk_email_bounce_soft']);
		$this->executeUpgradeQuery($tables['kmk_help_page']);
		$this->executeUpgradeQuery($tables['kmk_sitemap']);
		$this->executeUpgradeQuery($tables['kmk_thread_reply_ban']);
		...
		if ($regDefault)
		{
			...
			$this->_getDb()->update('kmk_option', array(
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_bb_code ADD allow_empty TINYINT UNSIGNED NOT NULL DEFAULT '0'
		");
		...
		");
		$this->executeUpgradeQuery("
			UPDATE kmk_forum SET moderate_threads = 1 WHERE moderate_replies = 1
		");

		$this->executeUpgradeQuery("RENAME TABLE kmk_trophy_user_title TO kmk_user_title_ladder");
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_user_title_ladder
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_poll ADD change_vote TINYINT UNSIGNED NOT NULL DEFAULT 0,
				ADD view_results_unvoted TINYINT UNSIGNED NOT NULL DEFAULT 1,
				ADD max_votes TINYINT UNSIGNED NOT NULL DEFAULT 1
		");
		$this->executeUpgradeQuery("
			UPDATE kmk_poll SET max_votes = 0 WHERE multiple = 1
		");
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_poll DROP multiple
		");
	}
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_user ADD activity_visible TINYINT UNSIGNED NOT NULL DEFAULT 1
		");
	}
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_user_profile ADD password_date INT UNSIGNED NOT NULL DEFAULT 1
		");
	}
}