...
class XenForo_Install_Upgrade_1020010 extends XenForo_Install_Upgrade_Abstract
{
	...
	public function step1()
	{
		...
		$this->executeUpgradeQuery('
			DROP TABLE IF EXISTS kmk_permission_cache_content_type
		');
		$this->executeUpgradeQuery('
			DROP TABLE IF EXISTS kmk_permission_cache_global_group
		');

		$this->executeUpgradeQuery($tables['kmk_deferred']);
		$this->executeUpgradeQuery($tables['kmk_draft']);
		$this->executeUpgradeQuery($tables['kmk_edit_history']);
		$this->executeUpgradeQuery($tables['kmk_forum_watch']);
		$this->executeUpgradeQuery($tables['kmk_registration_spam_cache']);
		$this->executeUpgradeQuery($tables['kmk_content_spam_cache']);
		$this->executeUpgradeQuery($tables['kmk_bb_code_parse_cache']);
		$this->executeUpgradeQuery($tables['kmk_route_filter']);
		$this->executeUpgradeQuery($tables['kmk_mail_queue']);
		
		$this->executeUpgradeQuery($tables['kmk_template_history']);
		$this->executeUpgradeQuery($tables['kmk_template_modification']);
		$this->executeUpgradeQuery($tables['kmk_template_modification_log']);
		$this->executeUpgradeQuery($tables['kmk_admin_template_modification']);
		$this->executeUpgradeQuery($tables['kmk_admin_template_modification_log']);
		$this->executeUpgradeQuery($tables['kmk_email_template_modification']);
		$this->executeUpgradeQuery($tables['kmk_email_template_modification_log']);
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE `kmk_session_activity` ADD `robot_key` VARBINARY(25) NOT NULL DEFAULT ''
		");
		...
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_ban_email CHANGE banned_email banned_email VARCHAR(120) NOT NULL
		");

		$this->executeUpgradeQuery("
			ALTER TABLE  `kmk_user_group`
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE `kmk_node` ADD `breadcrumb_data` BLOB NULL DEFAULT NULL
		");
		...
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_code_event_listener ADD hint VARCHAR(255) NOT NULL DEFAULT ''
		");
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE  `kmk_report` ADD  `report_count` INT UNSIGNED NOT NULL DEFAULT  '0'
		");
		$this->executeUpgradeQuery("
			ALTER TABLE `kmk_report_comment` ADD  `is_report` TINYINT UNSIGNED NOT NULL DEFAULT  '1'
		");

		$ids = $this->_getDb()->fetchCol("(SELECT user_id FROM kmk_moderator) UNION (SELECT user_id FROM kmk_admin)");
		if ($ids)
		{
			$this->executeUpgradeQuery("
				UPDATE kmk_report_comment SET is_report = 0 WHERE user_id IN (" . $this->_getDb()->quote($ids) . ")
			");
		}
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE `kmk_liked_content` ADD INDEX `like_user_content_type_id` (`like_user_id`, `content_type`, `content_id`)
		");
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE `kmk_user_profile` ADD INDEX  `dob` (  `dob_month` ,  `dob_day` ,  `dob_year` )
		");
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_user_option ADD enable_flash_uploader TINYINT UNSIGNED NOT NULL DEFAULT 1
		");

		$this->executeUpgradeQuery("
			UPDATE kmk_moderator AS moderator
			INNER JOIN kmk_user AS user ON (moderator.user_id = user.user_id)
			SET user.is_staff = 1
		");
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE kmk_conversation_master
				ADD recipients mediumblob NOT NULL,
				ADD INDEX user_id (user_id)
		");

		$this->executeUpgradeQuery("
			ALTER TABLE kmk_conversation_user
	...
	{
		$this->executeUpgradeQuery("
			ALTER TABLE `kmk_ip` ADD INDEX log_date (`log_date`)
		");

		$this->executeUpgradeQuery("
			INSERT IGNORE INTO kmk_option
