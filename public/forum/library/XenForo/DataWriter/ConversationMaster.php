...
class XenForo_DataWriter_ConversationMaster extends XenForo_DataWriter
{
	...
	*/
	protected function _getExistingData($data)
	{
		...
		return array('kmk_conversation_master' => $this->_getConversationModel()->getConversationMasterById($id));
	}
	...
	*/
	...
	public function addReply(array $reply)
	{
		if ($this->isUpdate())
		{
			...
			if ($master)
			{
				$this->_existingData['kmk_conversation_master'] = $master;
			}
		}
	...
	protected function _postSave()
	{
		...
		if ($this->_firstMessageDw)
		{
			...
			$this->_db->update('kmk_conversation_master', $toUpdate, 'conversation_id = ' . $this->_db->quote($conversationId));
	...
	protected function _postDelete()
	{
		...
		$db->query("
			UPDATE kmk_user AS user
			INNER JOIN kmk_conversation_user AS cuser ON
		...
		$db->delete('kmk_conversation_message', 'conversation_id = ' . $conversationIdQuoted);
		$db->delete('kmk_conversation_recipient', 'conversation_id = ' . $conversationIdQuoted);
		$db->delete('kmk_conversation_user', 'conversation_id = ' . $conversationIdQuoted);
		$db->delete('kmk_user_alert', 'content_type = \'conversation\' AND content_id = ' . $conversationIdQuoted);
