...
class XenForo_DataWriter_User extends XenForo_DataWriter
{
	...
	protected function _getFields()
	{
		return array(
			...
			'kmk_user_option' => array(
				'user_id'
					=> array('type' => self::TYPE_UINT, 'default' => array('kmk_user', 'user_id'), 'required' => true),
				'show_dob_year'
					=> array('type' => self::TYPE_BOOLEAN, 'default' => 1),
				'show_dob_date'
					=> array('type' => self::TYPE_BOOLEAN, 'default' => 1),
	...
	*/
	protected function _getExistingData($data)
	{
		...
		$returnData['kmk_user_authenticate'] = $this->_getUserModel()->getUserAuthenticationRecordByUserId($userId);
	...
	*/
	...
	public function setAlertOptOuts(array $optOuts)
	{
		...
		{
			$this->_db->delete('kmk_user_alert_optout', 'user_id = ' . $userId);

			foreach ($optOuts AS $alert)
			{
				$this->_db->insert('kmk_user_alert_optout', array(
					'user_id' => $userId,
					'alert' => $alert
				));
			}
	...
	public function setPassword($password, $passwordConfirm = false, XenForo_Authentication_Abstract $auth = null, $requirePassword = false)
	{
		...
		$this->set('data', $authData, 'kmk_user_authenticate');
		return true;
	}
	...
	*/
	...
	protected function _preSaveDefaults()
	{
		...
		if ($this->isChanged('scheme_class', 'kmk_user_authenticate') || $this->isChanged('data', 'kmk_user_authenticate'))
		{
	...
	protected function _preSave()
	{
		...
		if (!$this->get('scheme_class', 'kmk_user_authenticate') || !$this->get('data', 'kmk_user_authenticate'))
		{
			...
			$this->set('data', 'invalid', 'kmk_user_authenticate');
		}
				...
				foreach ($changes AS $table => $fields)
				{
					foreach ($fields AS $field => $newValue)
					{
						if (!in_array($field, self::$changeLogIgnoreFields))
						{
							...
							{
								if ($table == 'kmk_user_authenticate' && $field == 'data')
								{
									$oldValue = '*****';
									$newValue = '******';
								}
	...
	public function rebuildUserGroupRelations()
	{
		...
		$db->delete('kmk_user_group_relation', 'user_id = ' . $userIdQuoted);
		$db->query('
			INSERT INTO kmk_user_group_relation
	...
	public function updateCustomFields()
	{
		if ($this->_updateCustomFields)
		{
			...
			foreach ($this->_updateCustomFields AS $fieldId => $value)
			{
				...
				$this->_db->query('
					INSERT INTO kmk_user_field_value
	...
	protected function _postDelete()
	{
		...
		$db->delete('kmk_user_alert', "alerted_user_id = $userIdQuoted");
		$db->delete('kmk_user_alert_optout', "user_id = $userIdQuoted");
		$db->delete('kmk_user_change_log', "user_id = $userIdQuoted");
		$db->delete('kmk_user_change_temp', "user_id = $userIdQuoted");
		$db->delete('kmk_user_confirmation', "user_id = $userIdQuoted");
		$db->delete('kmk_user_external_auth', "user_id = $userIdQuoted");
		$db->delete('kmk_user_field_value', "user_id = $userIdQuoted");
		$db->delete('kmk_user_follow', "follow_user_id = $userIdQuoted");
        $db->delete('kmk_user_follow', "user_id = $userIdQuoted");
		$db->delete('kmk_user_group_change', "user_id = $userIdQuoted");
		$db->delete('kmk_user_group_promotion_log', "user_id = $userIdQuoted");
		$db->delete('kmk_user_group_relation', "user_id = $userIdQuoted");
		$db->delete('kmk_user_ignored', "ignored_user_id = $userIdQuoted");
        $db->delete('kmk_user_ignored', "user_id = $userIdQuoted");
		$db->delete('kmk_user_news_feed_cache', "user_id = $userIdQuoted");
		$db->delete('kmk_user_tfa', "user_id = $userIdQuoted");
		$db->delete('kmk_user_tfa_trusted', "user_id = $userIdQuoted");
		$db->delete('kmk_user_trophy', "user_id = $userIdQuoted");
		$db->delete('kmk_user_upgrade_active', "user_id = $userIdQuoted");
		$db->delete('kmk_user_upgrade_expired', "user_id = $userIdQuoted");
		$db->delete('kmk_warning', "user_id = $userIdQuoted");

		$db->delete('kmk_permission_combination', "user_id = $userIdQuoted");
		$db->delete('kmk_permission_entry', "user_id = $userIdQuoted");
		$db->delete('kmk_permission_entry_content', "user_id = $userIdQuoted");

		if ($this->get('is_moderator'))
		{
			$db->delete('kmk_moderator',  "user_id = $userIdQuoted");
			$db->delete('kmk_moderator_content',  "user_id = $userIdQuoted");
		}
		if ($this->get('is_admin'))
		{
			$db->delete('kmk_admin',  "user_id = $userIdQuoted");
			$db->delete('kmk_admin_permission_entry',  "user_id = $userIdQuoted");
		}
		if ($this->get('is_banned'))
		{
			$db->delete('kmk_user_ban',  "user_id = $userIdQuoted");
		}

		$db->delete('kmk_profile_post', "profile_user_id = $userIdQuoted");

		$db->delete('kmk_news_feed', "user_id = $userIdQuoted");

		$db->delete('kmk_conversation_user', "owner_user_id = $userIdQuoted");
		// note: leaving records in conversation recipient to keep data somewhat intact for others

		$db->delete('kmk_ip', "content_type = 'user' AND user_id = $userIdQuoted");
		// note: leaving content-associated IPs

		$db->delete('kmk_forum_read', "user_id = $userIdQuoted");
		$db->delete('kmk_thread_read', "user_id = $userIdQuoted");
		$db->delete('kmk_thread_reply_ban', "user_id = $userIdQuoted");
		$db->delete('kmk_thread_user_post', "user_id = $userIdQuoted");
		$db->delete('kmk_forum_watch', "user_id = $userIdQuoted");
		$db->delete('kmk_thread_watch', "user_id = $userIdQuoted");
