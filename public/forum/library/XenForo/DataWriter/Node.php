...
class XenForo_DataWriter_Node extends XenForo_DataWriter
{
	...
	*/
	protected function _getExistingData($data)
	{
		...
		return array('kmk_node' => $this->_getNodeModel()->getNodeById($nodeId));
	}
	...
	*/
	...
	protected function _verifyNodeName(&$data)
	{
		if (!$data)
		{
			if (!empty($this->_fields['kmk_node']['node_name']['required']))
			{
	...
	protected function _postDelete()
	{
		...
		if ($this->getOption(self::OPTION_POST_WRITE_UPDATE_CHILD_NODES))
		{
			...
			if ($nodeModel->hasChildNodes(array('lft' => $this->getExisting('lft'), 'rgt' => $this->getExisting('rgt'))))
			{
				...
				{
					$nodeModel->moveChildNodes($this->_existingData['kmk_node'], $moveToNodeId, false);
				}
				else
				{
					$nodeModel->deleteChildNodes($this->_existingData['kmk_node'], false);
				}
			}
