...
class XenForo_DataWriter_Option extends XenForo_DataWriter
{
	...
	*/
	protected function _getExistingData($data)
	{
		...
		return array('kmk_option' => $this->_getOptionModel()->getOptionById($id));
	}
	...
	*/
	...
	protected function _preSave()
	{
		...
		if ($this->isChanged('option_value'))
		{
			if ($this->getOption(self::OPTION_VALIDATE_VALUE))
			{
				...
				{
					$this->_setInternal('kmk_option', 'option_value', $optionValue);
				}
			}
	...
	protected function _postSave()
	{
		...
		{
			$db->update('kmk_option_group_relation',
	...
	protected function _updateRelatedGroupList(array $relations)
	{
		...
		if ($this->isUpdate())
		{
			$db->delete('kmk_option_group_relation', 'option_id = ' . $db->quote($this->getExisting('option_id')));
		}
		...
		foreach ($relations AS $groupId => $displayOrder)
		{
			...
			$db->insert('kmk_option_group_relation', array(
	...
	{
		$db = $this->_db;
		$db->delete('kmk_option_group_relation', 'option_id = ' . $db->quote($this->get('option_id')));
