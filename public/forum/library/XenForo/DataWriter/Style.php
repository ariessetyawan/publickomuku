...
class XenForo_DataWriter_Style extends XenForo_DataWriter
{
	...
	*/
	protected function _getExistingData($data)
	{
		...
		return array('kmk_style' => $this->_getStyleModel()->getStyleById($styleId));
	}
	...
	*/
	...
	protected function _postDelete()
	{
		...
		$db->delete('kmk_template_map', 'style_id = ' . $styleIdQuoted);
		$db->delete('kmk_template_compiled', 'style_id = ' . $styleIdQuoted);
		if (XenForo_Application::get('options')->templateFiles)
		{
			XenForo_Template_FileHandler::delete(null, $styleId, null);
		}
		$db->delete('kmk_style_property', 'style_id = ' . $styleIdQuoted);
		$db->delete('kmk_style_property_definition', 'definition_style_id = ' . $styleIdQuoted);
		$db->delete('kmk_style_property_group', 'group_style_id = ' . $styleIdQuoted);
		...
		$db->update('kmk_node', array(
			'style_id' => 0,
			'effective_style_id' => 0
		), "style_id = $styleIdQuoted OR effective_style_id = $styleIdQuoted");

		$db->update('kmk_user', array('style_id' => 0), "style_id = $styleIdQuoted");
