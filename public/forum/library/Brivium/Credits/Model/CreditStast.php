...
class Brivium_Credits_Model_CreditStast extends XenForo_Model
{
	...
	public function getStatisticRecords(array $conditions)
	{
		...
		return $this->fetchAllKeyed('
				...
				FROM kmk_maney_stats AS credits_stats
	...
	public function updateCreditStasts()
	{
		...
		$db->delete('kmk_maney_stats', ' currency_id NOT IN (' .$db->quote($currencyIds). ')');
	...
	public function deleteAllStast($currencyId)
	{
		...
		if($sqlConditions){
			...
			$this->_getDb()->query('DELETE FROM `kmk_maney_stats` WHERE ' . $whereConditions);
		}else{
			$this->_getDb()->query('TRUNCATE TABLE kmk_maney_stats');
		}
	}
