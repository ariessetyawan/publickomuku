...
class Brivium_Credits_Model_Credit extends XenForo_Model
{
	...
	protected function _pushToTransaction($record)
	{
		...
		$this->_getDb()->query('
			INSERT INTO kmk_maney_transaction
		...
		return $this->_getDb()->lastInsertId('kmk_maney_transaction', 'transaction_id');
	}
	...
	public function updateInterestAmount($userIds, $events)
	{
		...
		foreach($events AS $event){
			if(isset($event['amount'] ,$event['event_id']) && $event['amount'] && !empty($userIds[$event['event_id']])){
				...
				$db->query('
					UPDATE ' . (XenForo_Application::get('options')->BRC_enableUpdateLowPriority ? 'LOW_PRIORITY' : '') . ' kmk_user SET
	...
	/**
	 * import credits from other field of `kmk_user`
	 *
	 */
	public function importCredits($field,$column,$merge = true)
	{
		if (!$this->checkIfExist('kmk_user', $field)) {
			return false;
		}
		$merger = '';
		if($merge){
	...
	{
		$this->_getDb()->query('
			UPDATE kmk_user SET trophy_points = 0
		');
	}
	...
	{
		if(empty($column) || !$this->checkIfExist('kmk_user', $column)){
			return 0;
		}
