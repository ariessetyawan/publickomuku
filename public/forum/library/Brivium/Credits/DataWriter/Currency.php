...
class Brivium_Credits_DataWriter_Currency extends XenForo_DataWriter
{
	...
	*/
	protected function _getExistingData($data)
	{
		...
		return array('kmk_maney_currency' => $this->_getCurrencyModel()->getCurrencyById($currencyId));
	}
	...
	*/
	...
	protected function _verifyColumn($column)
	{
		...
		if(!$this->_checkIfExist('kmk_user',$column)){
			$this->_addColumn('kmk_user', $column, " decimal(19,6) NOT NULL DEFAULT '0.000000'");
		}
		return true;
	}
	...
	protected function _postSave()
	{
		//if($this->isChange('column')){
			//return $this->_db->query("ALTER TABLE  `kmk_user` CHANGE  `".$this->getExisting('column')."`  `".$this->get('column')."`");
		//}
	...
	protected function _postDelete()
	{
		...
		$db->delete('kmk_maney_event', "currency_id = $currencyidQuoted");
		$db->delete('kmk_maney_transaction', "currency_id = $currencyidQuoted");
		$db->delete('kmk_maney_stats', "currency_id = $currencyidQuoted");
