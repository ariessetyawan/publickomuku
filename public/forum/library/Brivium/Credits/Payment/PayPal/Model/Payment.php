...
class Brivium_Credits_Payment_PayPal_Model_Payment extends XenForo_Model
{
	...
	public function getTransactionLogById($id)
	{
		return $this->_getDb()->fetchRow('
			SELECT user.*, log.*
				FROM kmk_maney_paypal_log AS log
				LEFT JOIN kmk_user AS user ON (user.user_id = log.user_id)
	...
	{
		$this->_getDb()->insert('kmk_maney_paypal_log', $data);
