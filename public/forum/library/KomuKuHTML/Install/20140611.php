...
class KomuKuHTML_Install
{
    ...
    {
        self::_fetchDisabledInstallerKomuKuHTMLListeners($class);

        $createClass = XenForo_Application::resolveDynamicClass($class, 'installer_KomuKuHTML');
    ...
    protected function _updateJustInstalled()
    {
        if (isset($this->_data['addon_id'])) {
            if (XenForo_Application::$versionId < 1020000 &&
                 defined('KomuKuHTML_Listener_InitDependencies::JUST_INSTALLED_SIMPLE_CACHE_KEY')) {
                $justInstalled = XenForo_Application::getSimpleCacheData(
                    KomuKuHTML_Listener_InitDependencies::JUST_INSTALLED_SIMPLE_CACHE_KEY);
                ...
                XenForo_Application::defer('KomuKuHTML_Deferred',
    ...
    protected function _updateJustUninstalled()
    {
        if (isset($this->_data['addon_id'])) {
            if (XenForo_Application::$versionId < 1020000 &&
                 defined('KomuKuHTML_Listener_InitDependencies::JUST_UNINSTALLED_SIMPLE_CACHE_KEY')) {
                $justUninstalled = XenForo_Application::getSimpleCacheData(
                    KomuKuHTML_Listener_InitDependencies::JUST_UNINSTALLED_SIMPLE_CACHE_KEY);
     ...
     * @param string $hint
     */
    protected static function _fetchDisabledInstallerKomuKuHTMLListeners($hint = '')
    {
        ...
        $installerKomuKuHTMLListeners = XenForo_CodeEvent::getEventListeners('load_class_installer_KomuKuHTML');
        if (XenForo_Application::$versionId > 1020000) {
            if (isset($installerKomuKuHTMLListeners['_'])) {
                $installerKomuKuHTMLListeners = $installerKomuKuHTMLListeners['_'];
            } else {
                $installerKomuKuHTMLListeners = array();
            }
            if ($hint) {
                if (isset($installerKomuKuHTMLListeners[$hint])) {
                    $installerKomuKuHTMLListeners = array_merge($installerKomuKuHTMLListeners, $installerKomuKuHTMLListeners[$hint]);
                }
            }
        }
        foreach ($listeners as $listener) {
            if ($listener['event_id'] == 'load_class_installer_KomuKuHTML') {
                if ($installerKomuKuHTMLListeners) {
                    foreach ($installerKomuKuHTMLListeners as $installerKomuKuHTMLListener) {
                        list($callbackClass, $callbackMethod) = $installerKomuKuHTMLListener;
                ...
                if ($listener['active']) {
                    XenForo_CodeEvent::addListener('load_class_installer_KomuKuHTML',
                        array(
        ...
        $installerKomuKuHTMLListeners = XenForo_CodeEvent::getEventListeners('load_class_installer_KomuKuHTML');
    } /* END _fetchDisabledInstallerKomuKuHTMLListeners */
