...
class KomuKuHTML_Listener_ControllerPreDispatch
{
    ...
    const LAST_XML_UPLOAD_DATE_SIMPLE_CACHE_KEY = 'KomuKuHTML_lastXmlUploadDate';
    ...
    protected function _checkAddOnsNeedUpgrading(array $addOns)
    {
        ...
        if (is_subclass_of(self::$_controller, 'XenForo_ControllerAdmin_Abstract')) {
            eval(
                '
                class KomuKuHTML_Listener_ControllerPreDispatch_TemplatePostRender
                {
                	public static function templatePostRender($templateName, &$content, array &$containerData, XenForo_Template_Abstract $template)
                	{
                		if ($templateName == "PAGE_CONTAINER") {
                			$upgradeAddOns = KomuKuHTML_Listener_ControllerPreDispatch::getUpgradeAddOns();
                			if (!empty($upgradeAddOns)) {
            ...
            $newListener = array(
                'KomuKuHTML_Listener_ControllerPreDispatch_TemplatePostRender',
                'templatePostRender'
            );
            if (XenForo_Application::$versionId < 1020000) {
