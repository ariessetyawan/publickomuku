...
class EWRatendo_Model_Events extends XenForo_Model
{
	public function getEventsRange($first, $last)
	{
		...
		$events = $this->_getDb()->fetchAll("
			SELECT EWRatendo_events.*, kmk_user.*, IF(kmk_user.username IS NULL, EWRatendo_events.username, kmk_user.username) AS username
				FROM EWRatendo_events
				LEFT JOIN kmk_user ON (kmk_user.user_id = EWRatendo_events.user_id)
		...
		$recurs = $this->_getDb()->fetchAll("
			SELECT EWRatendo_events.*, kmk_user.*, EWRatendo_recurs.*, IF(kmk_user.username IS NULL, EWRatendo_events.username, kmk_user.username) AS username
	...
	public function getCurrentEvents($range = '+1 week', $nodeid = false, $thread = true)
	{
		if (!$events = $this->_getDb()->fetchAll("
			SELECT EWRatendo_events.*, kmk_user.*, kmk_thread.*,
				IF(kmk_user.username IS NULL, EWRatendo_events.username, kmk_user.username) AS username
			FROM EWRatendo_events
				LEFT JOIN kmk_user ON (kmk_user.user_id = EWRatendo_events.user_id)
				LEFT JOIN kmk_thread ON (kmk_thread.thread_id = EWRatendo_events.thread_id)
			WHERE EWRatendo_events.event_endtime >= ?
				AND EWRatendo_events.event_strtime <= ?
				AND EWRatendo_events.event_state = 'visible'
			". ($thread ? 'AND EWRatendo_events.thread_id != "0"' : '') ."
			". ($nodeid ? 'AND kmk_thread.node_id = "'.$nodeid.'"' : '') ."
