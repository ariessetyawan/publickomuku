...
class EWRatendo_Install
{
	...
	protected function _install_1()
	{
		...
		$this->_getDb()->query("INSERT IGNORE INTO `kmk_content_type` (`content_type`, `addon_id`, `fields`) VALUES ('event', 'EWRatendo', '')");
		$this->_getDb()->query("INSERT IGNORE INTO `kmk_content_type_field` (`content_type`, `field_name`, `field_value`) VALUES ('event', 'moderation_queue_handler_class', 'EWRatendo_ModerationQueueHandler_Event')");
	...
	protected function _uninstall_0()
	{
		...
		$this->_getDb()->query("DELETE IGNORE FROM `kmk_content_type` WHERE content_type = 'event'");
		$this->_getDb()->query("DELETE IGNORE FROM `kmk_content_type_field` WHERE content_type = 'event'");
		XenForo_Model::create('XenForo_Model_ContentType')->rebuildContentTypeCache();
	}
