...
class KomuKu_Install
{
	...
	public static function install()
	{
		...
		$wainAddon = str_replace('KomuKu_', 'Waindigo_', $data['addon_id']);

		$addOnKey = strtolower(str_replace('_','',$data['addon_id']));

		if ($data['addon_id'] != 'KomuKu_InstallUpgrade' && isset($xenAddons[$wainAddon]) && !isset($GLOBALS['th_upgrade'][$addOnKey])) {
			$addOn = XenForo_Model::create('XenForo_Model_AddOn')->getAddOnById($wainAddon);
			throw new XenForo_Exception('This add-on cannot be installed as you have '.$addOn['title'].' installed. Please either uninstall it first, or use [⌂] Install and Upgrade to upgrade to the new KomuKu version.', true);
		}
	...
	/**
	 *	Upgrade from KomuKu Add-on
	 */
	public static function upgradeFromKomuKu()
	{
		$args = func_get_args();
		if (isset($args[1])) {
			$data = $args[1];
	...
	{
		self::_fetchDisabledInstallerKomuKuListeners($class);
	...
	protected function _insertContentTypes(array $contentTypes)
	{
		...
		foreach ($contentTypes as $contentType => $contentTypeParams) {
			...
			if (isset($contentTypeParams['addon_id'])) {
				...
				$sql = "INSERT INTO kmk_content_type (
							content_type,
							addon_id,
							fields
						) VALUES (
				...
				$this->_db->update('kmk_content_type',
					array(
						'fields' => serialize($existingFields)
	...
	protected function _insertContentTypeFields(array &$contentTypeFields, array &$contentTypes = array())
	{
		foreach ($contentTypeFields as $contentType => $fields) {
			foreach ($fields as $fieldName => $fieldValue) {
				...
				$sql = "INSERT INTO kmk_content_type_field (
						content_type,
						field_name,
						field_value
					) VALUES (
	...
	protected function _deleteContentTypes(array $contentTypes)
	{
		foreach ($contentTypes as $contentType => $contentTypeParams) {
			if (isset($contentTypeParams['addon_id']) && $contentTypeParams['addon_id'] == $this->_data['addon_id']) {
				$addOnId = $contentTypeParams['addon_id'];
				$sql = "DELETE FROM kmk_content_type WHERE content_type = '" . $contentType . "' AND addon_id = '" .
					 $addOnId . "'";
				$this->_db->query($sql);
				$sql = "DELETE FROM kmk_content_type_field WHERE content_type = '" . $contentType . "'";
				$this->_db->query($sql);
			} else {
				if (isset($contentTypeParams['fields'])) {
	...
	{
		foreach ($nodeTypes as $nodeTypeId => $nodeTypeParams) {
			$sql = "DELETE FROM `kmk_node_type` WHERE `node_type_id` = '" . $nodeTypeId . "' ";
			$this->_db->query($sql);
			$sql = "DELETE FROM `kmk_node` WHERE `node_type_id` = '" . $nodeTypeId . "' ";
			$this->_db->query($sql);
		}
		XenForo_Model::create('XenForo_Model_Node')->rebuildNodeTypeCache();
	}
	...
	{
		if (self::_isAddOnInstalled('KomuKu_InstallUpgrade')) {
			if (method_exists('KomuKu_InstallUpgrade_ControllerHelper_InstallUpgrade', 'setFileName')) {
				KomuKu_InstallUpgrade_ControllerHelper_InstallUpgrade::setFileName($resourceUrl);
			}
		}
	}
	...
	protected function _updateJustInstalled()
	{
		if (isset($this->_data['addon_id'])) {
			if (XenForo_Application::$versionId < 1020000 &&
				 defined('KomuKu_Listener_InitDependencies::JUST_INSTALLED_SIMPLE_CACHE_KEY')) {
				$justInstalled = XenForo_Application::getSimpleCacheData(
					KomuKu_Listener_InitDependencies::JUST_INSTALLED_SIMPLE_CACHE_KEY);
				...
				XenForo_Application::setSimpleCacheData(
					KomuKu_Listener_InitDependencies::JUST_INSTALLED_SIMPLE_CACHE_KEY, $justInstalled);
			} elseif (XenForo_Application::$versionId >= 1020000) {
				XenForo_Application::defer('KomuKu_Deferred',
	...
	protected function _updateJustUninstalled()
	{
		if (isset($this->_data['addon_id'])) {
			if (XenForo_Application::$versionId < 1020000 &&
				 defined('KomuKu_Listener_InitDependencies::JUST_UNINSTALLED_SIMPLE_CACHE_KEY')) {
				$justUninstalled = XenForo_Application::getSimpleCacheData(
					KomuKu_Listener_InitDependencies::JUST_UNINSTALLED_SIMPLE_CACHE_KEY);
				...
				XenForo_Application::setSimpleCacheData(
					KomuKu_Listener_InitDependencies::JUST_UNINSTALLED_SIMPLE_CACHE_KEY, $justUninstalled);
			} elseif (XenForo_Application::$versionId >= 1020000) {
				XenForo_Application::defer('KomuKu_Deferred',
	 ...
	 * @param string $hint
	 */
	protected static function _fetchDisabledInstallerKomuKuListeners($hint = '')
	{
		...
		$installerKomuKuListeners = XenForo_CodeEvent::getEventListeners('load_class_installer_th');
		if (XenForo_Application::$versionId > 1020000) {
			if (isset($installerKomuKuListeners['_'])) {
				$installerKomuKuListeners = $installerKomuKuListeners['_'];
			} else {
				$installerKomuKuListeners = array();
			}
			if ($hint) {
				if (isset($installerKomuKuListeners[$hint])) {
					$installerKomuKuListeners = array_merge($installerKomuKuListeners,
						$installerKomuKuListeners[$hint]);
				}
			}
		}
		foreach ($listeners as $listener) {
			if ($listener['event_id'] == 'load_class_installer_th') {
				if ($installerKomuKuListeners) {
					foreach ($installerKomuKuListeners as $installerKomuKuListener) {
						list($callbackClass, $callbackMethod) = $installerKomuKuListener;
