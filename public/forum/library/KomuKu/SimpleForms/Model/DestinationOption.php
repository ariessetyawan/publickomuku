...
class KomuKu_SimpleForms_Model_DestinationOption extends XenForo_Model
{
	/**
	...
	public function getDestinationOptions(array $conditions = array(), array $fetchOptions = array())
	{
		...
		return $this->fetchAllKeyed('
			SELECT kmkform__destination_option.*
				' . $joinOptions['selectFields'] . '
			FROM kmkform__destination_option
			' . $joinOptions['joinTables'] . '
			WHERE ' . $whereClause . '
			ORDER BY kmkform__destination_option.display_order
		', 'option_id');
	}
	...
	{
		return $this->fetchAllKeyed('
			SELECT kmkform__destination_option.*, kmkform__form_destination_option.option_value
			...
			WHERE kmkform__form_destination_option.form_destination_id = ?
			ORDER BY kmkform__destination_option.display_order', 
	...
	public function getAttachmentsEnabled($formId)
	{
	    $result = $this->_getDb()->fetchOne('
            ...
            JOIN kmkform__form_destination_option
              ON kmkform__form_destination_option.form_destination_id = kmkform__form_destination.form_destination_id
	...
	public function prepareDestinationOptionConditions(array $conditions, array &$fetchOptions)
	{
		...
		{
			$sqlConditions[] = 'kmkform__destination_option.form_destination_id = ' . $db->quote($conditions['form_destination_id']);
		}
		*/
		
		if (!empty($conditions['destination_id']))
		{
			$sqlConditions[] = 'kmkform__destination_option.destination_id = ' . $db->quote($conditions['destination_id']);
		}
	...
	public function prepareDestinationOptionFetchOptions(array $fetchOptions)
	{
		...
		{
			$selectFields .= ',
				kmkform__form_destination_option.option_value';
			$joinTables .= '
				LEFT JOIN kmkform__form_destination_option ON
					(kmkform__form_destination_option.option_id = kmkform__destination_option.option_id AND kmkform__form_destination_option.form_id = ' . $db->quote($fetchOptions['valueFormId']) . ')';
		}
	...
	public function getDestinationOptionValues($formId)
	{
		$destinationOptions = $this->_getDb()->fetchAll('
			SELECT kmkform__form_destination_option.*, kmkform__destination_option.field_type
			FROM kmkform__form_destination_option
			INNER JOIN kmkform__destination_option ON (kmkform__destination_option.option_id = kmkform__form_destination_option.option_id)
			WHERE kmkform__form_destination_option.form_id = ?
		', $formId);
	...
	public function getDestinationOptionsInForm($formId, array $fetchOptions = array())
	{
		...
		return $this->fetchAllKeyed('
			SELECT kmkform__form_destination_option.*
				' . $joinOptions['selectFields'] . '
			FROM kmkform__form_destination_option
				' . $joinOptions['joinTables'] . '
			WHERE
