...
class KomuKu_SimpleForms_Model_Form extends XenForo_Model
{
	/**
	...
	public function getFormDestinationById($formDestinationId)
	{
		...
		return $this->_getDb()->fetchRow('
			...
			FROM kmkform__form_destination AS form_destination
	...
	public function prepareFormJoinOptions(array $fetchOptions)
	{
		...
		{
			$selectFields .= ",
				(SELECT COUNT(*) FROM kmkform__response AS response WHERE response.form_id = form.form_id) AS num_responses";
		}
		
		if (isset($fetchOptions['numUserResponses']))
		{
			$selectFields .= ",
				(SELECT COUNT(*) FROM kmkform__response AS response WHERE response.form_id = form.form_id AND response.user_id = " . $db->quote(XenForo_Visitor::getUserId()) . ") AS num_user_responses";
		}
	...
	public function prepareFormConditions(array $conditions, array &$fetchOptions)
	{
		...
		if (!empty($conditions['num_fields']))
		{
			...
			{
				$sqlConditions[] = "(SELECT COUNT(*) FROM kmkform__field WHERE form_id = form.form_id) " . $conditions['num_fields'][1] . " " . $conditions['num_fields'][0];
			}
		}
		
		if (!empty($conditions['max_responses']))
		{
			$sqlConditions[] = "form.max_responses = 0 OR (SELECT COUNT(*) FROM kmkform__response WHERE form_id = form.form_id) < form.max_responses";
		}
		...
		if (!empty($conditions['max_responses_per_user']))
		{
			...
			{
				$sqlConditions[] = 'form.max_responses_per_user = 0 OR (SELECT COUNT(*) FROM kmkform__response WHERE user_id = ' . $conditions['max_responses_per_user']['user_id'] . ') < form.max_responses_per_user';
			}
			else
			{
				$sqlConditions[] = 'form.max_responses_per_user = 0 OR (SELECT COUNT(*) FROM kmkform__response WHERE user_id = ' . $conditions['max_responses_per_user'] . ') < form.max_responses_per_user';
			}
		}
	...
	public function importFormXml(SimpleXMLElement $xml)
	{
		...
		$formDw = XenForo_DataWriter::create('KomuKu_SimpleForms_DataWriter_Form');
		...
		$pageDw = XenForo_DataWriter::create('KomuKu_SimpleForms_DataWriter_Page');
		...
		foreach ($xml->children() as $child)
		{
			switch ($child->getName())
			{
				case 'field':
				{
					...
					{
						$formFieldDw = XenForo_DataWriter::create('KomuKu_SimpleForms_DataWriter_Field');
				...
				case 'destination':
				{
					...
					{
						$formDestinationDw = XenForo_DataWriter::create('KomuKu_SimpleForms_DataWriter_FormDestination');
	...
	{
		$destinationOptionModel = XenForo_Model::create('KomuKu_SimpleForms_Model_DestinationOption');
	...
	{
		return XenForo_Model::create('KomuKu_SimpleForms_Model_Field');
	}
}