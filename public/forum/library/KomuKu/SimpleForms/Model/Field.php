...
class KomuKu_SimpleForms_Model_Field extends XenForo_Model
{
	/**
	 * Gets a field by ID.
	 *
	...
	public function getFields(array $conditions = array(), array $fetchOptions = array(), array $limitOptions = array())
	{
		...
		return $this->fetchAllKeyed('
			SELECT kmkform__field.*
				' . $joinOptions['selectFields'] . '
			FROM kmkform__field
			' . $joinOptions['joinTables'] . '
			WHERE ' . $whereClause . '
			ORDER BY kmkform__field.display_order
			' . $limit . '
		', 'field_id');
	}
	...
	public function moveFieldDisplayOrder($fieldId, $direction)
	{
		...
		$sql = '
			UPDATE `kmkform__field` SET `display_order` = IFNULL((
				SELECT * FROM (
					SELECT
						(SELECT
							CASE WHEN (' . $displayOrder . ') <= 0
								...
								ELSE (' . $displayOrder . ')
							END
						FROM `kmkform__field` `field`
						WHERE ' . $where . '
							AND `field`.`form_id` = `current`.`form_id`
						ORDER BY ' . $orderBy . '
						LIMIT 1)
					FROM `kmkform__field` `current`
	...
	public function getFieldsByResponseId($responseId)
	{
		return $this->fetchAllKeyed('
			...
			FROM kmkform__response_field response_field
			JOIN
				kmkform__field field ON field.field_id = response_field.field_id
	...
	public function getFieldValues($formId)
	{
		$fields = $this->_getDb()->fetchAll('
			...
			FROM kmkform__response_field AS response_field
			JOIN kmkform__field AS field ON (field.field_id = response_field.field_id)
			WHERE field.form_id = ?
		', $formId);
	...
	{
		$bind = array();
		$sql = "SELECT COUNT(*) FROM kmkform__field WHERE field_name = ? AND type = ?";
		$bind[] = $fieldName;
		$bind[] = $type;
	...
	{
		$this->_getDb()->query("
			UPDATE `kmkform__field` `field`
			JOIN `kmkform__field` `global`
	...
	public function prepareFieldConditions(array $conditions, array &$fetchOptions)
	{
		...
		{
			$sqlConditions[] = 'kmkform__field.form_id = ' . $db->quote($conditions['form_id']);
		}
		
		if (!empty($conditions['parent_field_id']))
		{
			$sqlConditions[] = 'kmkform__field.parent_field_id = ' . $db->quote($conditions['parent_field_id']);
		}
		...
		if (array_key_exists('type', $conditions))
		{
			...
			{
				$sqlConditions[] = "kmkform__field.type IN (" . $db->quote($conditions['type']) . ")";
			}
			else
			{
				$sqlConditions[] = "kmkform__field.type = " . $db->quote($conditions['type']);
			}
		}
		...
		if (array_key_exists('display_order', $conditions))
		{
			...
			{
				$sqlConditions[] = '`kmkform__field`.`display_order` ' . $conditions['display_order'][0] . ' ' . $conditions['display_order'][1];
			}
		}
		...
		switch ($field['field_type'])
		{
			...
			case 'datetime':
			    if ($value != '')
			    {
			        ...
			        if (count($temp) <> 2)
			        {
			            $error = new XenForo_Phrase('kmkform__please_enter_a_datetime_in_format');
			            return false;
			        }
			...
			case 'time':
		        ...
		        if (!$dateTime)
		        {
		            $error = new XenForo_Phrase('kmkform__please_enter_a_time_in_format');
		            return false;
		        }
			    break;
	...
	{
		$field = $this->_getDb()->fetchRow('SELECT * FROM `kmkform__field` WHERE `field_id` = ?', $fieldId);
		$field['field_name'] .= '_' . XenForo_Application::$time;
		$field['display_order'] += 10;
		$field['field_id'] = null;
		
		$this->_getDb()->insert('kmkform__field', $field);

		$newFieldId = $this->_getDb()->lastInsertId('kmkform__field');
		
		// save the field to make sure caches and phrases get updated
		$fieldModel = XenForo_Model::create('KomuKu_SimpleForms_Model_Field');
		$field = $fieldModel->getFieldById($newFieldId);
		
		$fieldDw = XenForo_DataWriter::create('KomuKu_SimpleForms_DataWriter_Field');
		$fieldDw->setExistingData($field);
		$fieldDw->setExtraData(KomuKu_SimpleForms_DataWriter_Field::DATA_TITLE, new XenForo_Phrase($this->getFieldTitlePhraseName($fieldId)));
		$fieldDw->setExtraData(KomuKu_SimpleForms_DataWriter_Field::DATA_DESCRIPTION, new XenForo_Phrase($this->getFieldDescriptionPhraseName($fieldId)));
		$fieldDw->save();
