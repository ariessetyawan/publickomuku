...
class KomuKu_SimpleForms_Option_LicenseKey
{
	/**
	...
	public static function verifyOption(&$licenseKey, XenForo_DataWriter $dw, $fieldName)
	{
		...
		{
			$errorMessage = 'The license key you entered is invalid. If you are having trouble with your license, please contact <a href="https://KomuKu.net/clients/clientarea.php" target="_blank">KomuKu Support</a>.';
	...
	public static function VerifyLicense($licenseKey, $localKey = null)
	{
		...
		{
			$localKey = $dataRegistry->get('kmkform__localkey');	
		}
		...
		if ($licenseKey != '')
		{
			...
			if (array_key_exists('status', $licenseCheck) && $licenseCheck['status'] == 'Active')
			{
				...
				{
					$dataRegistry->set('kmkform__localkey', $licenseCheck['localkey']);
				}
	...
	{
	    $dataRegistry->set('kmkform__localkey', '');
	}
	...
	protected static function _checkLicense($licenseKey, $localKey = '') {
	    ...
	    $domain = 'KomuKu.net';
	    $directory = '/clients';
	    $licensingSecretKey = "KomuKusSuperSecretPassKeyDONOTGIVEOUT!"; // Unique value, should match what is set in the product configuration for MD5 Hash Verification
