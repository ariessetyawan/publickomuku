...
class KomuKu_SimpleForms_Listener_Proxy
{
	/*
	...
	{
		$extend[] = 'KomuKu_SimpleForms_Listener_Proxy_AdminControllerForum';
	}
	
	public static function loadUserAdminController($class, array &$extend)
	{
		$extend[] = 'KomuKu_SimpleForms_Listener_Proxy_AdminControllerUser';
	}
	
	public static function loadUserGroupPromotionAdminController($class, array &$extend)
	{
		$extend[] = 'KomuKu_SimpleForms_Listener_Proxy_AdminControllerUserGroupPromotion';
	}

	public static function loadUserDataWriter($class, array &$extend)
	{
		$extend[] = 'KomuKu_SimpleForms_Listener_Proxy_DataWriterUser';
	}
	 
	public static function loadForumController($class, array &$extend)
	{
	    $extend[] = 'KomuKu_SimpleForms_Listener_Proxy_ControllerForum';
	}
	...
	{
		$extend[] = 'KomuKu_SimpleForms_Listener_Proxy_ModelForum';
	}
	...
	public static function controllerPreDispatch(XenForo_Controller $controller, $action)
	{
		...
		if ($controllerName == 'KomuKu_SimpleForms_ControllerAdmin_User' || $controllerName == 'KomuKu_SimpleForms_ControllerPublic_Forum')
		{
		    return true;
		}		
		...
		if ($licenseKey == '' && ($controllerName == 'XenForo_ControllerAdmin_Option' || substr($controllerName, 0, strlen('KomuKu_SimpleForms_Controller')) == 'KomuKu_SimpleForms_Controller'))
		{
			...
			$fileLicenseKeyValid = KomuKu_SimpleForms_Option_LicenseKey::VerifyLicense($fileLicenseKey);
			...
			{
				if (substr($controllerName, 0, strlen('KomuKu_SimpleForms_Controller')) == 'KomuKu_SimpleForms_Controller')
				{
					$controllerResponse = new XenForo_ControllerResponse_Error();
					$controllerResponse->errorText = 'You have not entered a license key. Please set your license key in the <a href="' . XenForo_Link::buildAdminLink('options/list', array('group_id' => 'KomuKu_SimpleForms')) . '">KomuKu Simple Forms options</a>.';
		...
		else if (substr($controllerName, 0, strlen('KomuKu_SimpleForms_Controller')) == 'KomuKu_SimpleForms_Controller')
		{
			KomuKu_SimpleForms_Option_LicenseKey::VerifyLicense($licenseKey);
		}
	}
	...
	{
		if (array_key_exists('forum', $params) && array_key_exists('kmkform__form_id', $params['forum']))
	    {
	        $params['form'] = self::_getFormModel()->getFormById($params['forum']['kmkform__form_id']);
	    }
	} 
