...
class KomuKu_SimpleForms_DataWriter_Helper_Field
{
	...
	public static function VerifyDefaultValue(&$defaultValue, XenForo_DataWriter $dw, $inputName = false)
	{
	    ...
	    if ($fieldType == 'datetime')
	    {
	        if (is_string($defaultValue))
	        {
	            ...
	            if (count($temp) <> 2)
	            {
	                $dw->error(new XenForo_Phrase('kmkform__please_enter_a_datetime_in_format'), $inputName);
	                return false;	                
	            }
	    ...
	    if ($fieldType == 'time')
	    {
            ...
            if (!$dateTime)
            {
                $dw->error(new XenForo_Phrase('kmkform__please_enter_a_time_in_format'), $inputName);
                return false;
            }
	    }	    
	    ...
	    if ($fieldType == 'date')
	    {
            ...
            if (!$dateTime)
            {
	            $dw->error(new XenForo_Phrase('kmkform__please_enter_a_date_in_format'), $inputName);
	            return false;
	        }
	    }
