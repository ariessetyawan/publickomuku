...
class KomuKu_SimpleForms_DataWriter_Response extends XenForo_DataWriter
{
	...
	/**
	 * Returns all kmkform__response fields, plus form-specific fields
	 */
	protected function _getFields()
	{
		return array(
			'kmkform__response' => array(
	...
	*/
	...
	public function preSave($checkAttachments = true)
	{
		...
		if ($checkAttachments)
		{
			...
			$attachments = $attachmentModel->getAttachmentsByTempHash($this->getExtraData(KomuKu_SimpleForms_DataWriter_Response::DATA_ATTACHMENT_HASH));

			if (count($attachments) < $form['require_attachment'])
			{
				$this->error(new XenForo_Phrase('kmkform__minimal_x_attachment_required_uploaded_y_z_more',
					array(
	...
	protected function _preDelete()
	{
		...
		$db->delete('kmkform__response_field', "response_id = $responseIdQuoted");
	...
	public function updateFields()
	{
		if ($this->_updateFields)
		{
			foreach ($this->_updateFields AS $fieldId => $field)
			{
				...
				$this->_db->query('
					INSERT INTO kmkform__response_field
	...
	/**
	 * @return KomuKu_SimpleForms_Model_Response
	 */
	protected function _getResponseModel()
	{
		return $this->getModelFromCache('KomuKu_SimpleForms_Model_Response');
	}	
