...
class KomuKu_BumpThread_ControllerPublic_Thread extends XFCP_KomuKu_BumpThread_ControllerPublic_Thread
{
	...
	private function checkForBumpTimeLimit(array $perms)
	{
		...
		if (!isset($perms['kmk_bump_thread_time']) || $perms['kmk_bump_thread_time'] <= 0)
		{
			return;
		}
		
		// max threads to bump. If not set (0), or unlimited (-1), switch to the max int value
		$maxThreadsToBump = isset($perms['kmk_bump_threads_total']) ? $perms['kmk_bump_threads_total'] : -1;
		...
		/* @var $bumpModel KomuKu_BumpThread_Model_BumpThread */
		$bumpModel = XenForo_Model::create('KomuKu_BumpThread_Model_BumpThread');
		$sinceTime = XenForo_Application::$time - $perms['kmk_bump_thread_time'] * 60;
		
		$data = $bumpModel->totalBumpedSince(XenForo_Visitor::getUserId(), $sinceTime);
		if ($data['totalBumped'] >= $maxThreadsToBump)
		{
			$allowedAtTime = $data['firstBumpDate'] + ($perms['kmk_bump_thread_time'] * 60); 
			...
			$msg = new XenForo_Phrase('kmk_bump_thread_time_disallowed', $params);
