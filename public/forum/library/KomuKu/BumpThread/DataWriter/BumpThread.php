...
class KomuKu_BumpThread_DataWriter_BumpThread extends XenForo_DataWriter
{
	...
	{
		$fields = array();
		$fields['kmk_bump_thread']['bump_id'] = array(
			'type' => self::TYPE_UINT,
			'required' => true,
			'autoIncrement' => true
		);		
		$fields['kmk_bump_thread']['user_id'] = array(
			'type' => self::TYPE_UINT,
			'required' => true
		);		
		$fields['kmk_bump_thread']['last_bump_thread'] = array(
			'type' => self::TYPE_UINT,
			'required' => true
		);
		$fields['kmk_bump_thread']['last_bump_date'] = array(
			'type' => self::TYPE_UINT,
			'required' => true,
	...
	protected function _getExistingData($key)
	{
		if ($key)
		{
			...
			$data = $db->fetchRow("SELECT * FROM kmk_bump_thread WHERE bump_id = ?", array( $key ));
			return array( 'kmk_bump_thread' => $data );
		}
		return null;
	}
