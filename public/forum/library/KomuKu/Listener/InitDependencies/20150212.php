...
class KomuKu_Listener_InitDependencies
{
	...
	protected function _checkJustInstalled()
	{
		...
		if ($justInstalled) {
			...
			foreach ($justInstalled as $addOnId) {
				if (method_exists('KomuKu_Install', 'postInstall')) {
					if (KomuKu_Install::postInstall(
						array(
							'addon_id' => $addOnId
						)) === false) {
						return false;
				...
				if (XenForo_Application::$versionId < 1020000) {
					...
					$db->update('kmk_code_event_listener',
						array(
							'active' => 1
	...
	protected function _checkJustUninstalled()
	{
		...
		if ($justUninstalled) {
			...
			foreach ($justUninstalled as $addOnId) {
				if (method_exists('KomuKu_Install', 'postUninstall')) {
					if (KomuKu_Install::postUninstall(
						array(
							'addon_id' => $addOnId
						)) === false) {
						return false;
	...
	protected function _rebuildLoadClassHintsCache()
	{
		...
		XenForo_CodeEvent::addListener('load_class', 'KomuKu_Listener_LoadClass', 'XenForo_ControllerPublic_Misc');
	}
	...
	public static function copyrightNotice(array $matches)
	{
		...
		return $matches[0] .
			...
			'Some XenForo functionality crafted by <a href="http://xf.KomuKu.io/" title="Premium XenForo Add-ons" target="_blank">KomuKu</a>.' .
			'</div>' .
			'</xen:if>';
	}
	...
	protected function _getLibraryListenerFileVersion($filename, $autoload = true)
	{
		...
		$version = 0;
		$handle = opendir($rootDir . '/KomuKu/Listener/' . $filename);
		if ($handle) {
			...
			if ($autoload) {
				require_once $rootDir . '/KomuKu/Listener/' . $filename . '/' . $version . '.php';
			}
		}
	 ...
	 * @param array $data
	 *
	 * @return KomuKu_Listener_InitDependencies
	 */
