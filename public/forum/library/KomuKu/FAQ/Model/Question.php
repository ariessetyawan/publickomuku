...
class KomuKu_FAQ_Model_Question extends XenForo_Model
{
    public function getById($faq_id, $moderation = 0)
    {
        return $this->_getDb()->fetchRow('
            SELECT f.*, c.title FROM kmk_faq_question f
            LEFT JOIN kmk_faq_category c ON (c.category_id = f.category_id)
    ...
    public function getQuestionsByIds(array $questionIds)
    {
        ...
        return $this->fetchAllKeyed('
            ...
            FROM kmk_faq_question AS question
            LEFT JOIN kmk_user AS user ON (user.user_id = question.user_id)
    ...
    public function getAll($fetchOptions = [])
    {
        ...
        return $this->fetchAllKeyed($this->limitQueryResults(
            'SELECT *, c.title
             FROM kmk_faq_question
             LEFT JOIN kmk_faq_category c ON (c.category_id = kmk_faq_question.category_id)
             WHERE moderation = 0
             '.$orderClause.'
    ...
    {
        $choices = [
            'question'      => 'sticky desc, kmk_faq_question.display_order asc, question',
            'view_count'    => 'sticky desc, kmk_faq_question.display_order asc, view_count',
            'submit_date'   => 'sticky desc, kmk_faq_question.display_order asc, submit_date',
        ];
    ...
    {
        return $this->fetchAllKeyed('SELECT * FROM kmk_faq_question WHERE category_id = ? ORDER BY submit_date DESC', $category_id);
    }

    public function logQuestionView($faq_id)
    {
        $this->_getDb()->query('UPDATE kmk_faq_question SET view_count = view_count+1 WHERE faq_id = ?', $faq_id);
    }

    public function deleteOrphanQuestions($category_id)
    {
        $this->_getDb()->query('DELETE FROM kmk_faq_question WHERE category_id = ?', $category_id);
    }
    ...
    public function getAndMergeAttachmentsIntoQuestion($questions, $faq_id = null)
    {
        ...
        if ($faq_id != null) {
            foreach ($attachmentModel->getAttachmentsByContentId('kmk_faq_question', $faq_id) as $attachmentId => $attachment) {
            ...
            if ($questionIds) {
                foreach ($attachmentModel->getAttachmentsByContentIds('kmk_faq_question', $questionIds) as $attachment) {
