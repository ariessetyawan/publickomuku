...
class KomuKu_FAQ_ModerationQueueHandler_Question extends XenForo_ModerationQueueHandler_Abstract
{
    public function getVisibleModerationQueueEntriesForUser(array $contentIds, array $viewingUser)
    {
        ...
        foreach ($questions as $question) {
            $output[$question['faq_id']] = [
                ...
                'contentTypeTitle' => new XenForo_Phrase('KomuKu_faq'),
            ];
        }
    ...
    public function approveModerationQueueEntry($contentId, $message, $title)
    {
        ...
        if ($question) {
            ...
            $alertModel->alert($question['user_id'], $visitor['user_id'], $visitor['username'], 'kmk_faq_question', $contentId, 'answered', $question);
        }

        // Remove from queue
        return $queueModel->deleteFromModerationQueue('kmk_faq_question', $contentId);
    }
    ...
    {
        return XenForo_Model::create('KomuKu_FAQ_Model_Question');
    }
}
