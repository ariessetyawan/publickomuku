...
class KomuKu_FAQ_Installer
{
    ...
    public static function install($existingAddOn, $addOnData)
    {
        ...
        if (!$existingAddOn) {
            $db->query(
                "CREATE TABLE IF NOT EXISTS `kmk_faq_question` (
            ...
            $db->query(
                "CREATE TABLE IF NOT EXISTS `kmk_faq_category` (
            ...
            $db->query(
                ...
                VALUES
                    ('kmk_faq_question', 'sitemap_handler_class', 'KomuKu_FAQ_SitemapHandler_Question'),
                    ('kmk_faq_question', 'attachment_handler_class', 'KomuKu_FAQ_AttachmentHandler_Question'),
                    ('kmk_faq_question', 'moderation_queue_handler_class', 'KomuKu_FAQ_ModerationQueueHandler_Question'),
                    ('kmk_faq_question', 'search_handler_class', 'KomuKu_FAQ_Search_DataHandler_Question'),
                    ('kmk_faq_question', 'alert_handler_class', 'KomuKu_FAQ_AlertHandler_Question'),
                    ('kmk_faq_question', 'like_handler_class', 'KomuKu_FAQ_LikeHandler_Question');"
            );
            ...
            if ($version < 101) {
                ...
                $db->query("UPDATE kmk_faq_question SET like_users='a:0:{}';");
            ...
            if ($version < 210) {
                ...
                $db->query("ALTER TABLE `kmk_faq_question` ADD COLUMN `sticky` tinyint NOT NULL DEFAULT '0' AFTER `moderation`;");
            }

            if ($version < 220) {
                $db->query("ALTER TABLE `kmk_faq_question` ADD COLUMN `display_order` int(10) unsigned NOT NULL DEFAULT '1' AFTER `moderation`;");
            }

            // Adding attachments
            if ($version < 250) {
                $db->query('ALTER TABLE `kmk_faq_question` ADD COLUMN `attach_count` int(10) AFTER `answer_date`;');

                $db->query('ALTER TABLE `kmk_faq_category` ADD COLUMN `short_desc` varchar(255) NOT NULL AFTER `display_order`, ADD COLUMN `long_desc` text NOT NULL AFTER `short_desc`;');
            ...
            if ($version < 310) {
                // Fix for upgrades
                $db->query("ALTER TABLE `kmk_faq_question` CHANGE COLUMN `display_order` `display_order` int(10) UNSIGNED NOT NULL DEFAULT '1';");
            }
