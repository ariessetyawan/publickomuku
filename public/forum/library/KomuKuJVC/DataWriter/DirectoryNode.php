...
*/
class KomuKuJVC_DataWriter_DirectoryNode extends XenForo_DataWriter
{
	/**
	...
	*/
	...
	/**
	 * Post-delete handler
	 * If there are child nodes of the deleted node, delete or move them accordingly
	 *
...
///////////////////////////////////  
	protected function _postDelete()
	{
		...
		if ($this->getOption(self::OPTION_POST_WRITE_UPDATE_CHILD_NODES))
		{
			...
			if ($nodeModel->hasChildNodes(array('lft' => $this->getExisting('lft'), 'rgt' => $this->getExisting('rgt'))))
			{
				...
				{
					$nodeModel->moveChildNodes($this->_existingData['kmk_jobvacan_directory_node'], $moveToNodeId, false);
				}
				else
				{
					$nodeModel->deleteChildNodes($this->_existingData['kmk_jobvacan_directory_node'], false);
				}
			}
	...
	// no need to rebuild permisions.. this was causing the forum to vanish when deleting directory node with children	
	...
	{
		// this doesnt need to be here, now node permisions for dir nodes
		/*
		$db = $this->_db;
		$nodeId = $this->get('node_id');
		$nodeIdQuoted = $db->quote($nodeId);

		$db->delete('kmk_permission_cache_content', "content_type = 'node' AND content_id = $nodeIdQuoted");
		$db->delete('kmk_permission_entry_content', "content_type = 'node' AND content_id = $nodeIdQuoted");
		*/
	}
	...
	
	{
		...
		// this doesnt need to be here, now mods for dir nodes
		/*
