...
class KomuKuJVC_Model_Directory extends XenForo_Model
{
	/**
	...
	public function updateNestedSetInfo()
	{
		...
		foreach ($nestedSetInfo AS $nodeId => $node)
		{
			...
			$writer = XenForo_DataWriter::create('KomuKuJVC_DataWriter_Directory');

			// we want to set nested set info, so don't prevent it
			$writer->setOption(KomuKuJVC_DataWriter_Directory::OPTION_ALLOW_NESTED_SET_WRITE, true);

			// prevent any child updates from occuring - we're handling it here
			$writer->setOption(KomuKuJVC_DataWriter_Directory::OPTION_POST_WRITE_UPDATE_CHILD_NODES, false);
	...
	// new method just to get Threads related to this directory
		public function getThreads($directoryId, array $conditions, array $fetchOptions = array())
	{
		...
		$query = '
				...
				AND dir.directory_category = '.$directoryId.'
