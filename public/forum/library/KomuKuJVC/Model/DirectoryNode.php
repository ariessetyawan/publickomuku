...
// methods similar to XenForo_Model_Node

class KomuKuJVC_Model_DirectoryNode extends XenForo_Model
{
	/**
	...
	public function getNodeById($nodeId, array $fetchOptions = array())
	{
		...
		$data = $this->_getDb()->fetchRow('
			...
			FROM kmk_jobvacan_directory_node AS node
	...
	public function getNodeTitleById($nodeId)
	{
		$data = $this->_getDb()->fetchRow('
			SELECT title 
			FROM kmk_jobvacan_directory_node AS node
	...
	public function getNodeTitlesByIds (array $nodeIds)
		{
	...
	public function getRootDirectories($ignoreNestedSetOrdering = false, $listView = false) 
	{	
		if ($ignoreNestedSetOrdering)
		{
			return $this->fetchAllKeyed('
	...
	public function getNodeBreadCrumbs(array $node, $includeSelf = true)
	{
		...
		foreach ($nodes AS $nodeId => $node)
		{
			$breadCrumbs[$nodeId] = array(
				'href' => XenForo_Link::buildPublicLink('full:' . 'directory/', $node),
				'value' => $node['title'],
	...
	public function getNodePermissionsForPermissionCombination($permissionCombinationId = null)
	{
		...
		if ($data === false)
		{
			...
			$permissionsData = $this->_getDb()->fetchOne('
				SELECT cache_value
				FROM kmk_permission_cache_content_type
	...
	public function getNodeTypeByNodeId($nodeId)
	{
		return $this->_getDb()->fetchRow('
			SELECT node_type.*
			FROM kmk_jobvacan_directory_node_type AS node_type
			INNER JOIN kmk_jobvacan_directory_node AS node ON
	...
	public function getAllNodeTypesFromDb()
	{
		return $this->fetchAllKeyed('
			SELECT *
			FROM kmk_jobvacan_directory_node_type
		', 'node_type_id');
	}
	...
	public function deleteChildNodes($parentNode, $rebuildCaches = true)
	{
		...
		if ($childNodes = $this->getChildNodes($parentNode))
		{
			foreach ($childNodes AS $childNodeId => $childNode)
			{
				// sometimes throws an error here for 1 person: "Undefined index: Directory"
				// why would $nodeTypes['Directory'] ever be empty?
				$dataWriterClass = $nodeTypes[$childNode['node_type_id']]['datawriter_class'];
	...
	public function updateNestedSetInfo()
	{
		...
		foreach ($nestedSetInfo AS $nodeId => $node)
		{
			...
			$writer = XenForo_DataWriter::create('KomuKuJVC_DataWriter_DirectoryNode');

			// we want to set nested set info, so don't prevent it
			$writer->setOption(KomuKuJVC_DataWriter_DirectoryNode::OPTION_ALLOW_NESTED_SET_WRITE, true);

			// prevent any child updates from occuring - we're handling it here
			$writer->setOption(KomuKuJVC_DataWriter_DirectoryNode::OPTION_POST_WRITE_UPDATE_CHILD_NODES, false);
	...
	public function getThreadsInDirectory($forumId, $directoryId, array $conditions = array(), array $fetchOptions = array())
	{
		$conditions['forum_id'] = $forumId;
		return $this->getThreads($directoryId, $conditions, $fetchOptions);
	}

	
	
	// new method just to get Threads and threadMaps related to this directory
	public function getThreads($directoryId, array $conditions, array $fetchOptions = array())
	{
		$threadModel = $this->getModelFromCache('XenForo_Model_Thread');
		
		$whereConditions = $threadModel->prepareThreadConditions($conditions, $fetchOptions);

		$sqlClauses = $threadModel->prepareThreadFetchOptions($fetchOptions);
		$limitOptions = $threadModel->prepareLimitFetchOptions($fetchOptions);
		$query = '
				SELECT dir.*, thread.*
					' . $sqlClauses['selectFields'] . '
				FROM kmk_thread AS thread
				' . $sqlClauses['joinTables'] . '
				LEFT JOIN kmk_jobvacan_thread_map AS dir ON (dir.thread_id = thread.thread_id)
				WHERE ' . $whereConditions . '
			
				AND 
				(
					dir.directory_category = '.$directoryId.' OR 
					dir.cat2 = '.$directoryId.' OR 
					dir.cat3 = '.$directoryId.' OR 
					dir.cat4 = '.$directoryId.' OR 
					dir.cat5 = '.$directoryId.'  
				)
				
				' . $sqlClauses['orderClause'];
		
		$thisReturn = $this->fetchAllKeyed($this->limitQueryResults(
